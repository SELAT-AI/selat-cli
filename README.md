...
| `selat budget [--json]` | Show the wallet's spending caps and remaining per-window budget (read-only). `selat budget start --amount <usd>` arms the **required** session tripwire for paid calls (`selat budget stop` disarms). Wallet caps are enforced at the money layer; `selat setup-policy` sets them. |
...
- **Params:** pass a skill's inputs as `--flags` (e.g. `--handle openai`).
  ...
  (`--max-amount` is hard-capped at $1 for non-TTY / agent runs).
...
1. **Per call** — `--max-amount` on `selat run` / `selat skill run` / `selat skill compare --pay`: a hard ceiling on any single payment. On `selat run`, the flag is applied last-wins after the catalog hint is validated. Default **$1** if omitted; an explicit value is **also** hard-capped at **$1**. Non-TTY / agent invocations cannot exceed $1. A TTY confirm or `--allow-high-max-amount` may raise the ceiling, still never above the documented **$5** hard cap. The Apify prepaid token (**$1.05**) is a documented exception for that known token only — not a generic hole. `selat skill run` takes `--max-amount` as a per-step override under the same ceiling. `selat skill compare --pay` defaults to live price + 25 % clamped to $1.
2. **Per session** — `selat budget start --amount <usd>`: a spending tripwire for the current work session (`selat budget stop` disarms; `selat budget` shows state). **Paid calls refuse unless a session budget is armed** (`selat run`, `selat skill run`, `compare --pay`, `verify --pay`). Free discovery (`search`, `doctor`, `skill list`) stays ungated. `selat freeze` is the kill switch.
