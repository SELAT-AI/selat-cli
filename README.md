...
```bash
selat skill compare "search the web for recent news" --live-probe --limit 3   # price/rail/latency/reliability, no spend
selat skill compare "token prices" --live-probe --pay --max-amount 0.05       # + one capped settled test call each (confirmed)
```
...
- **Params:** pass a skill's inputs as `--flags` (e.g. `--handle openai`).
  ...
  (`--max-amount` is hard-capped at $1.10 for everyone; `isTTY` does not raise it).
...
1. **Per call** — `--max-amount` on `selat run` / `selat skill run` / `selat skill compare --pay`: a hard ceiling on any single payment. On `selat run`, the flag is applied last-wins after the catalog hint is validated. Default **$1.10** if omitted; an explicit value is **also** hard-capped at **$1.10** for everyone (aligned with selat-pay). `isTTY` and `--allow-high-max-amount` do not raise it — agent harnesses often allocate a PTY, so those are not a human gate. The Apify prepaid token (**$1.05**) fits under that ceiling; an explicit `--max-amount` below $1.05 is still refused for Apify picks. `selat skill run` takes `--max-amount` as a per-step override under the same ceiling. `selat skill compare --pay` defaults to live price + 25 % clamped to $1.10.
