...
|---|---|
| `juris-cli init` | Full bootstrap. Idempotent — safe to re-run. |
| `juris-cli run "<intent>"` | Discover + rank + pay in one pipe. Sugar for the skill's `rank.mjs --pick \| jq \| sh` canonical flow. |
| `juris-cli fund [--chain ... --amount ...]` | Top up Gateway balance. Dry-runs first; requires explicit confirm. |
| `juris-cli setup-policy` | Set Circle spending caps on your Agent Wallet. Requires an email OTP (Circle's policy-write security). Recommended before any deposit > $20. |
| `juris-cli doctor` | Diagnose setup problems in one pass. Run when something looks off. |
| `juris-cli --help` | This page. |

## What this is

`juris-cli` is a thin orchestrator that bundles three pieces:
...
It doesn't reimplement any of them — it just wires them together so a new user can get from `juris-cli init` to their first paid response without hand-editing config files.
...
`juris-cli init` checks the required local tools, handles Circle login and wallet creation, and writes the config that `juris-pay` consumes. It does not install global dependencies on your behalf.

## Configuration

`juris-cli init` writes:
...
Override the router URL at init time: `juris-cli init --router-url=https://custom.example`

Override at runtime per call: `JURIS_ROUTER_URL=... juris-cli run "..."`

## Status

**v0.1.0 — scaffolding.** Init flow works against existing Circle CLI + juris-pay; welcome-drip step is stubbed (endpoint spec at [MyJuris/juris-router#6](https://github.com/MyJuris/juris-router/pull/6)). `juris-cli init` checks for `juris-pay` and prints guidance if it is not on `PATH`.
