...
test("skill compare hard-errors on unknown flags instead of silently dropping them", async () => {
  ...
  // discarded (falling back to the $1.10/candidate ceiling), --yes suppressed
...
test("skill run: firstPositional resolves the name past leading flags, incl. bare --json", async () => {
  ...
  // --max-amount ran under the manifest ceiling). Bare flags must not consume
  // the name as their value.
