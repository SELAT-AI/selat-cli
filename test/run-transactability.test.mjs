...
test("formats window key + successRate% + paidNum + lastPaid.status", () => {
  ...
  assert.equal(line, "Transactability Score: 7d · 98% delivered 2xx · 42 paid · last paid 200");
...
test("annotates low confidence when paidNum is small", () => {
  ...
  assert.equal(line, "Transactability Score: 24h · 100% delivered 2xx · 1 paid (low confidence) · last paid 200");
});
...
test("a non-2xx lastPaid is shown as-is, never as a bad badge", () => {
  ...
  assert.equal(line, "Transactability Score: 7d · 70% delivered 2xx · 8 paid · last paid 503");
});
