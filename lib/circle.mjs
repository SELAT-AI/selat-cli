...
// Cache TTLs for the two reads on the `selat run` hot path (see
// circle-cache.mjs for the invariant: never a spending gate). The wallet set
// only changes on createWallets (which invalidates); Gateway balances change
// on every deposit credit and paid call (fund / run / skill run invalidate).
export const WALLET_LIST_TTL_MS = 24 * 60 * 60 * 1000;
export const GATEWAY_BALANCE_TTL_MS = 5 * 60 * 1000;
export const GATEWAY_BALANCE_KEY_PREFIX = "gateway-balance:";
/** Cache key for one wallet's `gateway balance --all` read. Exported for doctor/tests. */
export function gatewayBalanceCacheKey(address, queryChain = "BASE") {
  return `${GATEWAY_BALANCE_KEY_PREFIX}${String(address).toLowerCase()}:${queryChain}`;
}

// The reads in this module signal failure with `null` / `readable: false` and
// never throw. That keeps callers simple, but it also erased WHY the Circle CLI
// failed — `selat doctor` could only say "Circle API issue" while the real
// stderr (rate limit, expired auth, network) went nowhere. Keep the most recent
// failure so diagnostics can print it. Best-effort, one slot, latest wins.
let lastReadError = null;
function noteReadError(what, r) {
  const detail = r == null
    ? "circle CLI could not be spawned"
    : (r.stderr || r.stdout || `exit ${r.code}`)
        .split(/\r?\n/)
        .map((l) => l.trim())
        // The Circle CLI prints Node deprecation notices on stderr; they are
        // not the failure.
        .filter((l) => l && !/DeprecationWarning|--trace-deprecation/.test(l))
        .join(" ")
        .slice(0, 300);
  lastReadError = `${what}: ${detail || `exit ${r?.code ?? "?"}`}`;
}
/** Most recent Circle read failure (`"<command>: <stderr>"`), or null. */
export function lastCircleReadError() {
...
/**
 * Oldest Circle CLI this stack works with. 1.1.1 was the first release that
 * lists Arc mainnet (ARC) and accepts it as a `gateway deposit` source; the
 * floor moved to 1.1.4 because Circle's own wallet-operations backend now
 * rejects 1.1.2/1.1.3 (`circle gateway balance` / `circle wallet balance`
 * error with "no longer supported ... Required: 1.1.4"). Mirrored by the
 * skill's `@circle-fin/cli` peer floor.
 */
export const MIN_CIRCLE_CLI_VERSION = "1.1.4";
...
export async function login(email) {
  ...
  // A cached wallet list carries no account identity, so a login that switches
  // Circle accounts would otherwise be handed the PREVIOUS account's wallets —
  // and `selat init` writes the address it is shown into config. Drop both
  // caches on any login attempt; they are cheap to rebuild and this is the one
  // moment the identity behind them can change.
  invalidateCircleCache("wallet-list:");
 ...
 */
export async function listAgentWallets({ cache = true } = {}) {
  return (await listAgentWalletsDetailed({ cache })).wallets;
}
 ...
 *
 * Eight sequential spawns at 4–17 s each, so a complete answer (every chain
 * read, at least one wallet) is cached for WALLET_LIST_TTL_MS; a partial
 * read or an empty account is never cached (init creates wallets right after
 * seeing "none", and a failed chain must be retried, not remembered).
 * `cache: false` reads live (doctor).
 */
export async function listAgentWalletsDetailed({ cache = true } = {}) {
  const key = `wallet-list:agent:${DEFAULT_QUERY_CHAINS.join(",")}`;
  if (!cache) return readAgentWalletsDetailed();
  return cachedCall(key, WALLET_LIST_TTL_MS, readAgentWalletsDetailed, {
    // Never cache a partial listing, and never cache a shape this version
    // cannot read back (init dereferences .wallets.length).
    cacheable: (r) =>
      r && Array.isArray(r.wallets) && Array.isArray(r.failures) &&
      r.failures.length === 0 && r.wallets.length > 0
  });
}

async function readAgentWalletsDetailed() {
 ...
 * Only the discovery fallback spawns `circle`; `cache: false` makes it live.
 */
export async function getAgentAddress({ cache = true } = {}) {
  const config = await readConfig().catch(() => ({}));
  const configured = configuredPayerAddress({ env: process.env, config });
  if (configured) return configured;
  const wallets = await listAgentWallets({ cache });
  const selected = await selectAgentWallet(wallets, {
    checkGateway: true,
    gatewayBalanceFor: (address) => gatewayBalance(address, "BASE", { cache })
  });
...
export async function createWallets({ fresh = false } = {}) {
  ...
  // The wallet set may have changed (even on a non-zero exit the CLI can have
  // created before failing) — the next `wallet list` must be live.
  invalidateCircleCache("wallet-list:");
  return r.code === 0;
}
 ...
 *
 * Base is the Eco (fast) deposit source (Circle CLI ≥1.0.0's eco coverage;
 * Optimism/Arbitrum are direct-only now); Polygon is the
 ...
 *
 * Membership rule (with lib/qr.mjs's EXTRA_FUND_CHAINS): a chain is fundable
 * iff it is BOTH Gateway-supported AND Agent-Wallet-supported — see the note
 * on FUND_QR_CHAINS in lib/qr.mjs for the rule, sources, and exclusions.
 */
...
export async function chainSpendingPolicy(address, chainCode) {
  ...
  if (!r || r.code !== 0) {
    noteReadError(`circle wallet limit --chain ${chainCode}`, r);
...
export async function walletUsdcBalance(address, chainCode) {
  ...
  if (!r || r.code !== 0) {
    noteReadError(`circle wallet balance --chain ${chainCode}`, r);
    return null;
  }
  try {
    return usdcFromWalletBalancePayload(JSON.parse(r.stdout));
  } catch {
    noteReadError(`circle wallet balance --chain ${chainCode}`, { code: 0, stdout: "output was not JSON" });
    return null;
  }
}
...
/**
 * Re-run a read that signals failure with `null` (the convention across this
 * module: swallowed errors surface as null, never throws). Circle API failures
 * come in correlated bursts — a run that saw one failed lookup tends to see the
 * next read fail too — and a single short-backoff retry clears most of them.
 * Callers must still handle a final null; retrying narrows the window, it does
 * not close it. `sleep` is injectable for tests.
 */
export async function retryNullRead(read, {
  attempts = 2,
  delayMs = 1500,
 ...
 */
export async function gatewayBalance(address, queryChain = "BASE", { cache = true } = {}) {
  const res = await gatewayBalancesByChain(address, queryChain, { cache });
 ...
 *
 * Successful reads are cached for GATEWAY_BALANCE_TTL_MS (a null is never
 * stored, so noteReadError below still reflects a real spawn). Pass
 * `cache: false` where the reading IS the proof — fund's pre/post-deposit
 * baseline and --wait polling, doctor — and invalidate (circle-cache.mjs)
 * after anything that moves money.
 */
/**
 * Account for a spend WITHOUT discarding the cached balance.
 *
 * Invalidating after every payment is what a first cut does, and it quietly
 * defeats the cache: the only writer is the chain resolver itself, so
 * run -> pay -> run re-spawns `circle gateway balance` (4-15 s) every time.
 * Instead, debit the cap the payment could not exceed. `--max-amount` is a
 * hard pre-signature ceiling, so subtracting it can only UNDER-state the
 * remaining balance, and under-stating is safe in the one place this value is
 * used: picking which funded chain to settle on. Too-low means we route to
 * another funded chain, or fall back to the catalog's hint — never that we
 * pick a chain which cannot cover the price.
 *
 * Call it BEFORE the paying spawn. The balance has already been read by then,
 * and an interrupt or a broadcast-then-fail must not leave a pre-money-move
 * snapshot behind (this repo already knows an interrupted payment can still
 * settle). The entry keeps its original storedAt, so this buys no extra life.
 *
 * Returns the number of cache entries adjusted (0 when nothing was cached).
 */
export function debitCachedGatewayBalance(address, chainKey, amountUsd) {
  const amount = Number(amountUsd);
  if (!address || !chainKey || !Number.isFinite(amount) || amount <= 0) return 0;
  const target = String(chainKey).toLowerCase();
  return adjustCircleCache(gatewayBalanceCacheKey(address, ""), (value) => {
    if (!value || !Array.isArray(value.perChain)) return null; // unexpected shape: drop, don't patch
    let hit = false;
    const perChain = value.perChain.map((row) => {
      if (gatewayNetworkToChainKey(row?.network) !== target) return row;
      hit = true;
      return { ...row, usdc: Math.max(0, Number(row.usdc ?? 0) - amount) };
    });
    if (!hit) return null; // paid on a chain we have no row for — re-read rather than guess
    const total = perChain.reduce((sum, row) => sum + Number(row.usdc ?? 0), 0);
    return { ...value, total, perChain };
  });
}

/**
 * The same, reading the chain and cap straight out of the argv about to be
 * spawned — that argv IS what selat-pay will enforce, and it is last-wins, so
 * the last occurrence of each flag is the operative one. Falls back to
 * invalidating when either is absent (an Apify token purchase, say): losing
 * the entry costs one spawn, guessing could cost a failed payment.
 */
export function debitCachedGatewayBalanceForSpawn(address, argv) {
  const last = (flag) => {
    const i = Array.isArray(argv) ? argv.lastIndexOf(flag) : -1;
    return i === -1 ? null : argv[i + 1] ?? null;
  };
  const chain = last("--chain");
  const cap = Number(last("--max-amount"));
  if (!address || !chain || !Number.isFinite(cap) || cap <= 0) {
    invalidateCircleCache(GATEWAY_BALANCE_KEY_PREFIX);
    return 0;
  }
  return debitCachedGatewayBalance(address, chain, cap);
}

export async function gatewayBalancesByChain(address, queryChain = "BASE", { cache = true } = {}) {
  if (!address) return null;
  if (!cache) return readGatewayBalancesByChain(address, queryChain);
  return cachedCall(
    gatewayBalanceCacheKey(address, queryChain),
    GATEWAY_BALANCE_TTL_MS,
    () => readGatewayBalancesByChain(address, queryChain),
    // Shape guard: a hand-edited file, or an entry written by a release with a
    // different return shape, must not be served to callers that dereference
    // it. Store only what this version can read back.
    { cacheable: (v) => v && typeof v.total === "number" && Array.isArray(v.perChain) }
  );
}

async function readGatewayBalancesByChain(address, queryChain) {
  const r = await sh(CIRCLE_BIN, [
    "gateway", "balance",
    "--address", address,
    "--chain", queryChain,
  ...
  if (!r || r.code !== 0) {
    noteReadError("circle gateway balance", r);
    return null;
  }
  try {
    const parsed = JSON.parse(r.stdout);
    ...
    if (rawTotal == null) {
      noteReadError("circle gateway balance", { code: 0, stdout: "no `total` in JSON output" });
      return null;
    }
    const toUsd = (v) => {
      const n = Number(v);
    ...
    noteReadError("circle gateway balance", { code: 0, stdout: "output was not JSON" });
    return null;
  }
}
