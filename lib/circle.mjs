...
/** The Circle CLI binary every caller shells out to (CIRCLE_BIN overrides). */
export const CIRCLE_BIN = process.env.CIRCLE_BIN || "circle";
...
async function spendingPolicyFromLimitRead(address, chainCode) {
  ...
  try {
    ...
    return {
      ...
      custom: true,
      limits: {
        perTx: finiteNumber(p.perTxLimit),
        daily: finiteNumber(p.dailyLimit),
        weekly: finiteNumber(p.weeklyLimit),
        monthly: finiteNumber(p.monthlyLimit)
      }
    };
  } catch {
...
export async function spendingBudget(address, chainCode = "BASE") {
  ...
  try {
    ...
    const rows = [];
    if (pol.per_tx_limit != null) rows.push({ window: "per-tx", limit: finiteNumber(pol.per_tx_limit), remaining: null });
    for (const w of ["daily", "weekly", "monthly"]) {
      if (pol[`${w}_limit`] != null || pol[`${w}_remaining`] != null) {
        rows.push({ window: w, limit: finiteNumber(pol[`${w}_limit`]), remaining: finiteNumber(pol[`${w}_remaining`]) });
      }
    }
