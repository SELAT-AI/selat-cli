 ...
 */
/**
 * Read the wallet's Circle spending policy (`circle wallet limit`). The policy
 * is the one hard spending ceiling the agent literally cannot bypass, so
 * money-moment commands surface it. Returns { readable: false } when the CLI
 * or read is unavailable, else { readable: true, custom, limits } where
 * `custom` means a user-set (origin CUSTOM) transfer-limit policy exists and
 * `limits` is { perTx, daily, weekly, monthly } in USD (null when unset).
 * Read-only; never throws. Policy reads need a mainnet chain; BASE default.
 */
