 ...
 * `SELAT_AGENT_WALLET_ADDRESS` (see selat-pay `--address` default). A Circle
 * account can hold up to 5 agent wallets with DISTINCT addresses (createWallets
 * `{ fresh: true }`), so deriving the address purely from `listAgentWallets()`
 * can resolve a *different* (and possibly unfunded) wallet than the payer —
 * which makes a funded wallet look empty ("no funded Gateway balance to
 * auto-select"). So honor the configured payer address first; only fall back to
 * discovery (most-chains wallet) when it is unset.
 */
export async function getAgentAddress() {
  const configured = (process.env.SELAT_AGENT_WALLET_ADDRESS ?? "").trim();
