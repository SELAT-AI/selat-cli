...
import { sh, shSafe, hasBin } from "./sh.mjs";
...
export async function authStatus() {
  const r = await shSafe(CIRCLE_BIN, ["wallet", "status", "--type", "agent"]);
  if (r.code !== 0) {
    debugError("circle wallet status", new Error(r.stderr || `exit ${r.code}`));
    return { authed: false };
  }
...
export async function listAgentWalletsDetailed() {
  ...
  for (const chain of DEFAULT_QUERY_CHAINS) {
    const r = await shSafe(CIRCLE_BIN, ["wallet", "list", "--type", "agent", "--chain", chain, "--output", "json"]);
    if (r.code !== 0) {
      failures.push({
        chain,
        code: r.code,
    ...
    try {
      ...
      debugError(`parsing circle wallet list (${chain}) output`, err);
...
export async function getAgentAddress() {
  // A config read that fails for any reason other than "not there yet" must be
  // said out loud: the fallback below resolves a DIFFERENT wallet than the one
  // selat-pay signs with, which reads as a funded wallet mysteriously showing
  // $0 (issue #54). readConfig() already returns {} when the file is absent.
  const config = await readConfig().catch((err) => {
    debugError("reading selat-pay config", err);
    console.error(fmt.warn(
      `could not read ${configPath()} (${errorChain(err)}) — falling back to wallet discovery, ` +
      "which may resolve a different wallet than selat-pay signs with"
    ));
    return {};
  });
 ...
 */
/**
 * Read the wallet's Circle spending policy. The policy is the one hard
 * spending ceiling the agent literally cannot bypass, so money-moment
 * commands surface it. Returns { readable: false } when the CLI or read is
 * unavailable, else { readable: true, custom, limits } where `custom` means
 * a user-set (origin CUSTOM) transfer-limit policy exists and `limits` is
 * { perTx, daily, weekly, monthly } in USD (null when unset).
 *
 * Source of truth is the `circle wallet limit budget` surface, NOT the plain
 * `circle wallet limit` read. Verified live (2026-07, dev wallet): the plain
 * `limit` read is inconsistent across query chains — it reports origin
 * DEFAULT with null limits on POLYGON while reporting the user's CUSTOM caps
 * on BASE — whereas `limit budget` reports the SAME origin CUSTOM, the SAME
 * limits, and the SAME consumed counters on every chain (it tracks Gateway
 * spending and is wallet-scoped). A clarifying question is pending with
 * Circle; until answered, `limit budget` is the truthful cap-state surface
 * and the plain `limit` read is only a fallback for when budget is
 * unreadable.
 *
 * `chainCode` is INTERNAL query routing only (the Circle CLI insists on a
 * --chain even for wallet-scoped reads; BASE default). The policy is part of
 * the user's money model, and the money model is chain-free: no caller may
 * surface a chain name from this result.
 *
 ...
 * used when `limit budget` is unreadable (see spendingPolicy() for why the
 * budget surface is the source of truth).
 */
async function spendingPolicyFromLimitRead(address, chainCode) {
  const r = await shSafe(CIRCLE_BIN, [
    "wallet", "limit",
    "--address", address,
    "--chain", chainCode,
    "--output", "json"
  ...
  if (r.code !== 0) {
    debugError("circle wallet limit", new Error(r.stderr || `exit ${r.code}`));
    return { readable: false };
  }
  try {
    const parsed = JSON.parse(r.stdout);
    const policies = parsed?.data?.policies ?? parsed?.policies ?? [];
    const p = Array.isArray(policies)
      ? policies.find((x) =>
          String(x?.origin ?? "").toUpperCase() === "CUSTOM" &&
          String(x?.ruleType ?? "").toUpperCase() === "TRANSFER_LIMIT")
      : null;
    if (!p) return { readable: true, custom: false };
    const num = (v) => { const n = Number(v); return Number.isFinite(n) ? n : null; };
    return {
      readable: true,
      custom: true,
      limits: { perTx: num(p.perTxLimit), daily: num(p.dailyLimit), weekly: num(p.weeklyLimit), monthly: num(p.monthlyLimit) }
    };
  } catch (err) {
    debugError("parsing circle wallet limit output", err);
    return { readable: false };
  }
}
 ...
 * `chainCode` is internal query routing only (see spendingPolicy) — the
 * budget is wallet-scoped and its consumed counters read identically on
 * every chain (verified live).
 */
export async function spendingBudget(address, chainCode = "BASE") {
  ...
  if (r.code !== 0) {
    debugError("circle wallet limit budget", new Error(r.stderr || `exit ${r.code}`));
    return { readable: false };
  }
  try {
    const parsed = JSON.parse(r.stdout);
    ...
    debugError("parsing circle wallet limit budget output", err);
    return { readable: false };
  }
}

/**
 * One human line for a spendingPolicy() result — shared by fund, doctor, and
 * budget so the wallet's cap state reads the same everywhere.
 */
export function describePolicy(policy) {
  ...
  return parts.length ? `capped at ${parts.join(" \u00b7 ")}` : "custom policy set";
}
...
export async function walletUsdcBalance(address, chainCode) {
  ...
  if (r.code !== 0) {
    debugError(`circle wallet balance (${chainCode})`, new Error(r.stderr || `exit ${r.code}`));
    return null;
  }
  try {
    return usdcFromWalletBalancePayload(JSON.parse(r.stdout));
  } catch (err) {
    debugError(`parsing circle wallet balance (${chainCode}) output`, err);
    return null;
  }
}
...
export async function gatewayBalancesByChain(address, queryChain = "BASE") {
  ...
  if (r.code !== 0) {
    debugError("circle gateway balance", new Error(r.stderr || `exit ${r.code}`));
    return null;
  }
  try {
    const parsed = JSON.parse(r.stdout);
    ...
    debugError("parsing circle gateway balance output", err);
    return null;
  }
}
