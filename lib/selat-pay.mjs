...
import { shSafe, hasBin } from "./sh.mjs";
...
export async function resolveSelatPay() {
  ...
  try {
    ...
    debugError("resolving the bundled @selat-ai/selat-pay", err);
  }
...
export async function ensureSelatPayHistoryDir() {
  const historyPath = process.env.SELAT_PAY_HISTORY_PATH ||
    join(process.env.XDG_STATE_HOME || join(homedir(), ".local", "state"), "selat-pay", "gateway-history.jsonl");
...
export async function selatPayVersion() {
  ...
  if (r.packageRoot) {
    try {
      ...
      debugError("reading the selat-pay package.json", err);
    }
  }
  ...
  const res = await shSafe(cmd, args);
  if (res.code !== 0) {
    debugError("smoke-checking selat-pay --help", new Error(res.stderr || `exit ${res.code}`));
    return null;
  }
