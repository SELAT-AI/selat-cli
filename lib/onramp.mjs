 ...
 */
export function onrampSessionLines({ widgetUrl, minutesLeft = null, walletEmail = null }) {
  return [
    ...
    // Surfaced with EVERY widget link (agents relay these lines verbatim):
    // the widget's KYC continues on the user's phone, and using a different
    // email there than the Circle Agent Wallet login breaks the link between
    // the verified identity and the wallet being funded.
    "KYC: the identity check may continue on your phone — when it asks for an email, " +
      (walletEmail
        ? `use the SAME email as your Circle Agent Wallet login: ${walletEmail}`
        : "use the SAME email as your Circle Agent Wallet login")
  ];
}
