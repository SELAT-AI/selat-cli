...
async function listCmd(args) {
  ...
  if (skills.length === 0 && broken.length === 0) {
  ...
  // Installed-but-unloadable folders used to be dropped from this list, which
  // read as "never installed" and sent users to reinstall a skill that was
  // already on disk.
  for (const b of broken) {
    console.error(fmt.warn(`${b.name ?? b.dir}: installed but not loadable — ${b.error}`));
  }
  return broken.length > 0 && skills.length === 0 ? 1 : 0;
}
...
async function verifyCmd(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (key === "chain") overrides.chain = value === true ? undefined : value;
...
async function payCompareRows(sorted, { intent, selatPay, maxAmount, chain, yes }) {
  ...
  for (const [i, { row, capUsd }] of jobs.entries()) {
    ...
    if (res.stdout.trim()) {
      ...
      const target = join(sampleDir, `${i + 1}-${slug}.${ext}`);
      try {
        await writeFile(target, res.stdout, "utf8");
        samplePath = target;
      } catch (err) {
        // The call already settled: losing the results table (and the remaining
        // paid candidates) to a failed sample write would throw away what the
        // user just paid for.
        debugError(`writing the paid sample to ${target}`, err);
        console.error(fmt.warn(`could not save the response sample to ${target}: ${errorChain(err)}`));
      }
    }
    row.paid = {
      settled: res.settled,
