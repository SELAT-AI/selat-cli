...
const BARE_SKILL_FLAGS = new Set(["--force", "--pay", "--dry-run", "--json", "--raw-key"]);
...
async function compareCmd(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a.startsWith("--")) {
      console.error(fmt.error(`unknown flag ${a} — known flags: --limit, --max-amount, --chain, --json, --pay, --yes, --refresh, --live-probe`));
      return 1;
    }
    words.push(a);
  }
...
const SKILL_RUN_USAGE = `usage: selat skill run <name> [--param value ...] [--chain <chain>] [--max-amount <usd>] [--raw-key] [--json]
...
Flags:
  ...
  --max-amount <usd>  Override the per-step spend cap
