...
${fmt.bold("Subcommands:")}
  ...
  run <name> [--p value ...]    Run an installed skill; pass its params as --flags. --json emits\n                                one machine object (per-step responses + a user_summary line).
                                Overrides: --chain <c>, --max-amount <usd>, --allow-high-max-amount.
...
const BARE_SKILL_FLAGS = new Set(["--force", "--pay", "--dry-run", "--json", ALLOW_HIGH_MAX_AMOUNT_FLAG]);
...
const SKILL_RUN_USAGE = `usage: selat skill run <name> [--param value ...] [--chain <chain>] [--max-amount <usd>] [${ALLOW_HIGH_MAX_AMOUNT_FLAG}] [--json]
