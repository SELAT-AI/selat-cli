...
async function verifyCmd(args) {
  ...
  if (pay && overrides.maxAmount != null) {
    const raised = await confirmHighMaxAmount({
      explicit: overrides.maxAmount,
      interactive: stdinIsInteractive() && !jsonMode,
      allowHigh: !!overrides.allowHighMaxAmount,
      jsonMode,
      prompt: (msg) => promptYesNo(msg, { default: false }),
    });
    if (!raised.ok) {
      if (jsonMode) console.log(JSON.stringify({ schema: "selat-verify/v1", ok: false, error: raised.error }));
      else {
        console.error(fmt.error(raised.error));
        if (raised.hint) console.error(fmt.dim(raised.hint));
      }
      return 1;
    }
    const auth = authorizeExplicitMaxAmount(overrides.maxAmount, {
      interactive: stdinIsInteractive() && !jsonMode,
      allowHigh: raised.allowHigh,
    });
    if (!auth.ok) {
    ...
    overrides.allowHighMaxAmount = raised.allowHigh;
...
${fmt.bold("Flags:")}
  ...
  --max-amount <usd> Per-call spend cap for --pay (default: live price + 25%
                     ...
                     value is also hard-capped at $${HARD_CLI_MAX_AMOUNT_USD} unless a
                     TTY confirm or ${ALLOW_HIGH_MAX_AMOUNT_FLAG} raises it
                     (still ≤ $${RAISED_CLI_MAX_AMOUNT_USD}).
  ${ALLOW_HIGH_MAX_AMOUNT_FLAG}
                     TTY only: raise --max-amount up to $${RAISED_CLI_MAX_AMOUNT_USD}
...
async function compareCmd(args) {
  ...
  if (pay && maxAmount != null) {
    ...
    allowHighMaxAmount = raised.allowHigh;
  }
...
Flags:
  ...
  --max-amount <usd>  Override the per-step spend cap (hard CLI ceiling $${HARD_CLI_MAX_AMOUNT_USD};
                      non-TTY / agent runs cannot exceed that)
  ${ALLOW_HIGH_MAX_AMOUNT_FLAG}
                      TTY only: raise --max-amount up to $${RAISED_CLI_MAX_AMOUNT_USD}
...
async function runCmd(args) {
  ...
  if (overrides.maxAmount != null) {
    ...
    if (!raised.ok) {
      if (jsonMode) console.log(JSON.stringify({ ok: false, error: raised.error }));
      else {
        console.error(fmt.error(raised.error));
        if (raised.hint) console.error(fmt.dim(raised.hint));
      }
