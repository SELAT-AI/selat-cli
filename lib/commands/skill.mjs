...
import { ensureSelatPayHistoryDir, resolveSelatPay } from "../selat-pay.mjs";
...
import { missingSessionBudget } from "./budget.mjs";
import {
...
${fmt.bold("Subcommands:")}
  ...
  run <name> [--p value ...]    Run an installed skill; pass its params as --flags. --json emits\n                                one machine object (per-step responses + a user_summary line).
                                Overrides: --chain <c>, --max-amount <usd>, --allow-high-max-amount, --raw-key.
                                Paid: requires an armed session budget (\`selat budget start\`).
  compare "<intent>" [--limit N] [--json] [--pay]
                                ...
                                and requires an armed session budget.
...
${fmt.bold("Examples:")}
  ...
  selat skill compare "search the web for recent news" --live-probe --limit 3
...
async function installCmd(args) {
  ...
  const maxAmount = flag(args, "--max-amount") || undefined;
...
const BARE_SKILL_FLAGS = new Set(["--force", "--pay", "--dry-run", "--json", "--raw-key", ALLOW_HIGH_MAX_AMOUNT_FLAG]);
...
async function verifyCmd(args) {
  const path = firstPositional(args);
  if (!path) {
    console.error(fmt.error("usage: selat skill verify <path> --live-probe [--pay] [--param value ...]"));
    return 1;
  }
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (key === "live-probe") { liveProbe = true; continue; }
    // Skipped before the value read below so it can't eat the next token.
    if (key === "json") { jsonMode = true; continue; }
    if (key === ALLOW_HIGH_MAX_AMOUNT_FLAG.slice(2)) { overrides.allowHighMaxAmount = true; continue; }
    const next = args[i + 1];
  ...
  if (!liveProbe) {
    const message = "selat skill verify may invoke each step endpoint; pass --live-probe only after confirming the targets are safe to call";
    if (jsonMode) console.log(JSON.stringify({ schema: "selat-verify/v1", ok: false, error: message }));
    else console.error(fmt.error(message));
    return 1;
  }

  if (pay && overrides.maxAmount != null) {
    const auth = authorizeExplicitMaxAmount(overrides.maxAmount, {
      interactive: stdinIsInteractive() && !jsonMode,
      allowHigh: !!overrides.allowHighMaxAmount,
    });
    if (!auth.ok) {
      if (jsonMode) console.log(JSON.stringify({ schema: "selat-verify/v1", ok: false, error: auth.error }));
      else {
        console.error(fmt.error(auth.error));
        if (auth.hint) console.error(fmt.dim(auth.hint));
      }
      return 1;
    }
    overrides.maxAmount = auth.capUsd;
    overrides.allowHighMaxAmount = false;
    overrides.interactive = stdinIsInteractive() && !jsonMode;
  }

  if (pay) {
    const unarmed = missingSessionBudget();
    if (unarmed) {
      if (jsonMode) console.log(JSON.stringify({ schema: "selat-verify/v1", ok: false, error: unarmed.error, hint: unarmed.hint }));
      else {
        console.error(fmt.error(unarmed.error));
        console.error(fmt.dim(unarmed.hint));
      }
      return 1;
    }
  }
  ...
  try {
    res = await verifySkillDir(path, {
      params, overrides, pay, liveProbe,
      // Progress lines would corrupt a JSON stream; --json consumers get the
      // receipt only.
      onStep: jsonMode ? undefined : ({ index, total, phase, step, display }) => {
        if (phase === "probe") {
          const label = step.label ? ` — ${step.label}` : "";
          console.log(fmt.dim(`▸ step ${index + 1}/${total}${label}`));
          console.log(fmt.dim(`  probe: selat-pay ${display} --probe-only --live-probe`));
        } else if (phase === "pay") {
    ...
    if (jsonMode) {
      // stdout stays machine-readable even on failure — same schema id, ok:false.
      console.log(JSON.stringify({ schema: "selat-verify/v1", ok: false, error: err.message ?? String(err), ...(err.paramKey ? { missingParam: err.paramKey } : {}) }, null, 2));
      return 1;
    }
    if (err.paramKey) {
  ...
  if (jsonMode) {
    // The canonical selat-verify/v1 receipt — byte-identical in shape to the
    // .selat/verify-receipt.json file and to what `skill submit` reads, so
    // there is exactly one machine format for verification results.
    console.log(JSON.stringify({ ...res.receipt, receiptPath: res.receiptPath }, null, 2));
    return res.ok ? 0 : 1;
  }
  ...
  console.log("");
  const unreachable = diagnoseUnreachable(res.steps);
  if (unreachable?.kind === "setup") {
    console.log("  " + fmt.err(unreachable.message));
    console.log("  " + fmt.cyan(unreachable.hint));
    console.log("");
  }
...
${fmt.bold("Usage:")}
  selat skill compare "<intent>" --live-probe [flags]

${fmt.bold("Flags:")}
  --live-probe       Required. Acknowledges that probing fetches each
                     candidate's live 402 at its catalog serviceUrl, which may
                     invoke the target method/body (never settles). Without it
                     the command refuses and probes nothing.
  ...
  --pay              One capped SETTLED test call per reachable candidate
                     ...
                     response as an output sample. Requires an armed session
                     budget (\`selat budget start\`).
  ...
  --max-amount <usd> Per-call spend cap for --pay (default: live price + 25%
                     headroom, clamped to $${HARD_CLI_MAX_AMOUNT_USD}). An explicit
                     value is also hard-capped at $${HARD_CLI_MAX_AMOUNT_USD} for
                     everyone — isTTY and ${ALLOW_HIGH_MAX_AMOUNT_FLAG} do not
                     raise it.
  ${ALLOW_HIGH_MAX_AMOUNT_FLAG}
                     Accepted for compatibility; does not raise the $${HARD_CLI_MAX_AMOUNT_USD} ceiling
  --chain <c>        Pay-chain override for --pay (probes are chain-independent)
  --help, -h         Show this help

${fmt.bold("Examples:")}
  selat skill compare "search the web for recent news" --live-probe
  selat skill compare "enrich a person by name and company" --live-probe --limit 3
  selat skill compare "generate an image" --live-probe --json
  selat skill compare "token prices" --live-probe --pay --max-amount 0.05
...
async function compareCmd(args) {
  ...
  // approach as `selat search`). Unknown --flags are an ERROR — with --pay
  // this command spends money, and silently dropping a typo'd flag like
  // --max-ammount discards the user's cap (falling back to the $1.10/candidate
  // ceiling) while its value pollutes the intent. Same rationale as run.mjs.
  let limit = 5;
  let maxAmount = null;
  let chain = null;
  let json = false;
  ...
  let refresh = false;
  let liveProbe = false;
  let allowHighMaxAmount = false;
  const words = [];
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "--limit") { limit = Number(args[++i]); continue; }
    if (a === "--max-amount") { maxAmount = args[++i]; continue; }
    if (a === "--chain") { chain = args[++i]; continue; }
    if (a === "--json") { json = true; continue; }
    if (a === "--pay") { pay = true; continue; }
    if (a === "--yes") { yes = true; continue; }
    if (a === ALLOW_HIGH_MAX_AMOUNT_FLAG) { allowHighMaxAmount = true; continue; }
    if (a === "--refresh") { refresh = true; continue; }
    if (a === "--live-probe") { liveProbe = true; continue; }
    if (a.startsWith("--")) {
      console.error(fmt.error(`unknown flag ${a} — known flags: --limit, --max-amount, ${ALLOW_HIGH_MAX_AMOUNT_FLAG}, --chain, --json, --pay, --yes, --refresh, --live-probe`));
      return 1;
    }
    words.push(a);
  }
  ...
  if (!intent) {
    console.error(fmt.error("usage: selat skill compare \"<intent>\" --live-probe [--limit N] [--json] [--pay]"));
    return 1;
  }
  if (!liveProbe) {
    console.error(fmt.error("selat skill compare may invoke each candidate endpoint; pass --live-probe only after confirming those targets are safe to call"));
    console.error(fmt.dim(`  retry: selat skill compare "${intent}" --live-probe${limit !== 5 ? ` --limit ${limit}` : ""}`));
    return 1;
  }
  ...
  if (pay && maxAmount != null) {
    const auth = authorizeExplicitMaxAmount(maxAmount, {
      interactive: stdinIsInteractive() && !json,
  ...
  const discovery = findSkill("rank.mjs");
  if (!discovery.found) {
    console.error(fmt.error("discovery skill not found"));
    console.error("");
    for (const line of skillInstallLines(discovery.path)) console.error(fmt.cyan(line));
    return 1;
  }
  ...
  let found;
  try {
    found = await discoverCandidates(join(discovery.path, "scripts", "rank.mjs"), intent, { limit, refresh });
  } catch (err) {
    console.error(fmt.error(err.message ?? String(err)));
  ...
  console.error(fmt.dim(`▸ live-probing ${found.plans.length} candidate${found.plans.length > 1 ? "s" : ""} at their catalog serviceUrl (no spend; target methods may run)…`));
  ...
  if (pay) {
    const code = await payCompareRows(sorted, { intent, selatPay, maxAmount, chain, yes, allowHigh: allowHighMaxAmount });
  ...
  // Diagnostics go to stderr, so --json stdout stays machine-clean.
  const unreachable = diagnoseUnreachable(sorted);
  if (unreachable) {
    console.error("");
    console.error(fmt.error(unreachable.message));
    if (unreachable.hint) console.error(fmt.cyan(`  ${unreachable.hint}`));
    return 1;
  }
  return 0;
}
 ...
 */
async function payCompareRows(sorted, { intent, selatPay, maxAmount, chain, yes, allowHigh = false }) {
  const jobs = [];
  for (const row of sorted) {
    ...
    if (!Number.isFinite(capUsd)) {
      console.error(fmt.error(`--max-amount ${maxAmount} exceeds the hard CLI ceiling`));
      return 1;
    }
  ...
  if (frozen) {
    console.error(fmt.error(frozen.error));
    console.error(fmt.dim(frozen.hint));
    return 1;
  }
  const unarmed = missingSessionBudget();
  if (unarmed) {
...
const SKILL_RUN_USAGE = `usage: selat skill run <name> [--param value ...] [--chain <chain>] [--max-amount <usd>] [${ALLOW_HIGH_MAX_AMOUNT_FLAG}] [--raw-key] [--json]

Run an installed skill's steps, paying per step from your funded Gateway balance.
Requires an armed session budget (\`selat budget start\`).

Flags:
  --<param> <value>   Fill a manifest param (repeatable; manifest keys map 1:1)
  --chain <chain>     Override the pay chain for every step
  --max-amount <usd>  Override the per-step spend cap (hard CLI ceiling $${HARD_CLI_MAX_AMOUNT_USD};
                      isTTY and agent harnesses cannot exceed that)
  ${ALLOW_HIGH_MAX_AMOUNT_FLAG}
                      Accepted for compatibility; does not raise the $${HARD_CLI_MAX_AMOUNT_USD} ceiling
  --raw-key           Sign locally with SELAT_PRIVATE_KEY (chains the Agent Wallet can't sign)
  --json              Machine-readable output
  -h, --help          Show this help. Never resolves, never pays.`;

async function runCmd(args) {
  // Help must be inert on the one spend subcommand too. Every sibling money
  // command (run, fund, setup-policy, skill verify/compare) prints help and
  // returns BEFORE any resolve or spend. Without this guard, `--help` fell
  // through the flag loop below as a pass-through skill param (resolveParams
  // deliberately allows unknown params) and `-h` was skipped as a non-`--`
  // token — either way the skill's steps executed and PAID. Same defect class
  // the run.mjs "help must be inert" note fixed there.
  if (args.includes("--help") || args.includes("-h")) {
    console.log(SKILL_RUN_USAGE);
    return 0;
  }

  // firstPositional, not args[0]: `selat skill run --json my-skill` used to
  // take "--json" as the skill name. Every other skill subcommand already
  // resolves its target this way.
  const name = firstPositional(args);
  if (!name) {
  ...
  // Parse trailing flags. --chain, --max-amount, and --raw-key are reserved
  // overrides applied to every step; everything else becomes a skill param
  // (manifest keys map 1:1, no camelization). A bare --flag with no value is
  // `true`. --raw-key signs locally with SELAT_PRIVATE_KEY (selat-pay derives
  // the EOA from the key), needed for chains the Circle Agent Wallet can't sign
  // (e.g. Arc mainnet, where the Gateway balance sits under a raw EOA).
  const params = {};
  const overrides = {};
  let jsonMode = false;
  // Iterate ALL args; the name is a non-flag token skipped by the `continue`
  // below wherever it sits. (Was args.slice(1), which dropped whatever was at
  // position 0 — the name only when no flag preceded it.)
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (!a.startsWith("--")) continue;
    const key = a.slice(2);
    // Bare toggles never consume the following token (which could be a param
    // value, or the skill name itself).
    if (key === "json") { jsonMode = true; continue; }
    if (key === "raw-key") { overrides.rawKey = true; continue; }
    if (key === ALLOW_HIGH_MAX_AMOUNT_FLAG.slice(2)) { overrides.allowHighMaxAmount = true; continue; }
    const next = args[i + 1];
    const value = next != null && !next.startsWith("--") ? args[++i] : true;
    if (key === "chain") overrides.chain = value === true ? undefined : value;
  ...
  if (overrides.maxAmount != null) {
    ...
    if (!auth.ok) {
      if (jsonMode) console.log(JSON.stringify({ ok: false, error: auth.error }));
      else {
        console.error(fmt.error(auth.error));
        if (auth.hint) console.error(fmt.dim(auth.hint));
      }
  ...
  if (!found) {
    if (jsonMode) {
      console.log(JSON.stringify({ ok: false, error: `skill '${name}' is not installed`, install: `selat skill install ${name}` }));
      return 1;
    }
  ...
  try {
    ...
    // --json callers are machines: the error must be parseable on stdout,
    // matching verifyCmd's {ok:false, error} contract — not prose on stderr
    // with an empty stdout.
    if (err.paramKey) {
      if (jsonMode) {
        console.log(JSON.stringify({ ok: false, error: `missing required --${err.paramKey}`, param: err.paramKey, ...(err.paramDescription ? { description: err.paramDescription } : {}) }));
        return 1;
      }
      console.error(fmt.error(`missing required --${err.paramKey}`));
      if (err.paramDescription) console.error(fmt.dim(`  ${err.paramDescription}`));
    ...
    if (jsonMode) {
      console.log(JSON.stringify({ ok: false, error: err.message ?? String(err), ...(err.hint ? { hint: err.hint } : {}) }));
      return 1;
    }
    console.error(fmt.error(err.message ?? String(err)));
    if (err.hint) console.error(fmt.dim(err.hint));
    return 1;
  }
}
