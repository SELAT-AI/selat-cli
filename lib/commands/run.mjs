...
const RUN_USAGE = "usage: selat run \"<intent>\" [--endpoint <url> [--method <verb>]] [--max-amount <usd>] [--dry-run] [--live-probe] [--param key=value ...] [--input '<json>' | --input-file <path>] [--auto-rebuy] [--json] [--verbose]";

// Every flag `selat run` understands. Kept as data so the unknown-flag error
// can list them and tests can pin the set.
export const KNOWN_RUN_FLAGS = ["--dry-run", "--live-probe", "--param", "--input", "--input-file", "--auto-rebuy", "--endpoint", "--method", "--max-amount", "--json", "--verbose"];

// Conservative per-call ceiling when `selat run` is not given --max-amount.
// Catalog exec_hints are untrusted; a hint above this is clamped. An explicit
// --max-amount is user authority and is not clamped. Same $1 bound as
// skill-registry's manifest ceiling and skill compare --pay.
export const DEFAULT_RUN_MAX_AMOUNT_USD = 1;
...
export function parseRunArgs(args) {
  const opts = { inputInline: undefined, inputFile: undefined, autoRebuy: false, dryRun: false, liveProbe: false, jsonMode: false, verbose: false, rawParams: [], endpoint: undefined, method: undefined, maxAmount: undefined };
  const intentTokens = [];
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--endpoint" || a === "--method" || a === "--max-amount") {
      const value = args[++i];
      if (value == null || value.startsWith("--")) {
        return { ok: false, error: `${a} requires a value${value != null ? ` (got flag-like ${value})` : ""}` };
      }
...
export async function run(args) {
  ...
  if (parsedArgs.help) {
    ...
    console.log(`                        catalog hint is validated (default $${DEFAULT_RUN_MAX_AMOUNT_USD}`);
    console.log("                        if omitted; an explicit value is user authority)");
    console.log("  --json                Machine-readable output");
    console.log("  --verbose             Show rank/pay diagnostics");
    console.log("  -h, --help            Show this help. Never ranks, never pays.");
    return 0;
  }
  const { intent, inputInline, inputFile, autoRebuy, dryRun, liveProbe, jsonMode, verbose, rawParams, endpoint, method, maxAmount } = parsedArgs;
  if (!intent) {
  ...
  const pick = await sh(
    "node",
    [join(skill.path, "scripts", "rank.mjs"), intent, "--pick", ...(liveProbe ? ["--live-probe"] : []), ...pinArgs({ endpoint, method })]
  );
  if (pick.code !== 0) {
  ...
  if (isApifyPrepaidPick(plan)) {
    ...
    return await runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy, intent, dryRun, jsonMode, verbose, maxAmount });
  }
  ...
  const capped = applyRunMaxAmountCeiling(selatPayArgs, { explicit: maxAmount });
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false, intent = "", dryRun = false, jsonMode = false, verbose = false, maxAmount } = {}) {
  ...
  // make every Apify pick unpayable.
  if (maxAmount != null && maxAmount < 1.05) {
    return emitRunError({
      jsonMode,
      error: `Apify prepaid-token purchase is $1.05 — --max-amount ${maxAmount} is below that; nothing was charged`,
    });
  }
 ...
 *   - explicit --max-amount is user authority (not clamped)
 *   - otherwise min(hint's last cap, DEFAULT_RUN_MAX_AMOUNT_USD), or the
 *     default when the hint has no usable cap
 */
export function applyRunMaxAmountCeiling(argv, { explicit = null, defaultUsd = DEFAULT_RUN_MAX_AMOUNT_USD } = {}) {
  ...
  if (explicit != null) {
    ...
    if (!Number.isFinite(cap) || cap <= 0) {
      return { ok: false, reason: `--max-amount "${explicit}" must be a positive number` };
    }
