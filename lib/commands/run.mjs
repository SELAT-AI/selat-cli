...
import { stdinIsInteractive, promptYesNo } from "../ui.mjs";
import {
...
// unless a TTY confirm or --allow-high-max-amount raises it (still ≤ $5).
// Same $1 bound as skill-registry's manifest ceiling and skill compare --pay.
...
export async function run(args) {
  ...
  if (parsedArgs.help) {
    ...
    console.log("                        even when the flag is explicit. Non-TTY / agent runs");
    ...
    console.log(`                        TTY only: raise the per-call ceiling up to $${RAISED_CLI_MAX_AMOUNT_USD}`);
    console.log("                        (same as a TTY confirm). Still cannot exceed that hard cap");
  ...
  // Refuse a YOLO 999 before ranking — $5 is the documented hard cap even on a
  // TTY. The $1-vs-$1.05 Apify exception is decided after the pick, so a
  // 1.05 flag is not judged here (that would be a generic hole if we allowed
  // it on every x402 call, and a false refuse if we blocked it on Apify).
  if (maxAmount != null && maxAmount > RAISED_CLI_MAX_AMOUNT_USD) {
    return emitRunError({
      jsonMode,
      error: `--max-amount ${maxAmount} exceeds the documented hard cap $${RAISED_CLI_MAX_AMOUNT_USD} (nothing was charged)`,
    });
  }
...
/**
 * TTY / flag authorization for an explicit `--max-amount` on `selat run`.
...
export async function authorizeRunMaxAmount({
  ...
  const auth = authorizeExplicitMaxAmount(maxAmount, {
    interactive,
    allowHigh: raised.allowHigh || allowHighMaxAmount,
    apifyPrepaid,
  });
  if (!auth.ok) return auth;
  return { ...auth, allowHigh: !!(raised.allowHigh || (allowHighMaxAmount && interactive)) };
}
 ...
 *     A TTY raise / --allow-high-max-amount may lift that ceiling to $5.
 *     Values above the allowed ceiling are refused (not silently spent).
