...
import { ensureSelatPayHistoryDir, resolveSelatPay, selatPaySpawn, SELAT_PAY_MISSING_MESSAGE } from "../selat-pay.mjs";
...
const RUN_USAGE = "usage: selat run \"<intent>\" [--dry-run] [--param key=value ...] [--input '<json>' | --input-file <path>] [--auto-rebuy] [--json] [--verbose]";

// Every flag `selat run` understands. Kept as data so the unknown-flag error
// can list them and tests can pin the set.
export const KNOWN_RUN_FLAGS = ["--dry-run", "--param", "--input", "--input-file", "--auto-rebuy", "--json", "--verbose"];
...
export function parseRunArgs(args) {
  const opts = { inputInline: undefined, inputFile: undefined, autoRebuy: false, dryRun: false, jsonMode: false, verbose: false, rawParams: [] };
  const intentTokens = [];
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--param") {
      const value = args[++i];
      if (value == null) return { ok: false, error: "--param requires a key=value argument" };
...
export async function run(args) {
  ...
  const { intent, inputInline, inputFile, autoRebuy, dryRun, jsonMode, verbose, rawParams } = parsedArgs;
  if (!intent) {
  ...
  if (!parsedParams.ok) {
    console.error(fmt.error(parsedParams.error));
    return 1;
  }
  const params = parsedParams.params;

  const skill = requireSkill("rank.mjs");
  if (!skill) return 1;
  ...
  // Step 1: pick
  const pick = await sh("node", [skillScript(skill, "rank.mjs"), intent, "--pick"]);
  if (pick.code !== 0) {
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false, intent = "", dryRun = false, jsonMode = false, verbose = false }) {
  ...
  const tokenScript = skillScript(skill, "apify_token.mjs");
...
async function replanWithParams({ plan, skill, intent, params }) {
  ...
  const { url } = applyParamsToUrl(epUrl, params);
  ...
  const libPath = skillScript(skill, "lib", "rank.mjs");
  ...
  let rebuilt;
  try {
    rebuilt = lib.buildPaymentPlan(intent, result, {});
  } catch (err) {
    return { ok: false, reason: `re-planning with params failed: ${err.message}` };
  }
  if (rebuilt?.requiresPathParams || rebuilt?.requiresQueryParams) {
...
function reportMissingParams({ plan, intent, jsonMode, missing = [], reason }) {
  ...
  for (const m of missing) {
    const bits = [`  ${m.name}`, m.kind === "path" ? "(path param)" : "(query param)"];
