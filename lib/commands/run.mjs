...
export async function run(args) {
  ...
  // per-call x402 with no token to replace. Every other --flag is dropped.
  let inputInline;
  let inputFile;
  let autoRebuy = false;
  const rawParams = [];
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--auto-rebuy") { autoRebuy = true; continue; }
    if (a === "--param") { rawParams.push(args[++i]); continue; }
    if (a.startsWith("--")) continue;
    intentTokens.push(a);
  }
  const intent = intentTokens.join(" ").trim();
  if (!intent) {
    console.error(fmt.error("usage: selat run \"<intent>\" [--param key=value ...] [--input '<json>' | --input-file <path>] [--auto-rebuy] [--json] [--verbose]"));
    return 1;
  }
  ...
  if (isApifyPrepaidPick(plan)) {
    ...
    return await runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy, intent, jsonMode, verbose });
  }
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false, intent = "", jsonMode = false, verbose = false }) {
