...
// for everyone — isTTY and --allow-high-max-amount cannot raise it. Same
// $1.10 bound as selat-pay HARD_PAY_MAX_AMOUNT_USD and skill compare --pay.
// (skill-registry's untrusted-manifest ceiling stays $1.)
...
export async function run(args) {
  ...
  if (parsedArgs.help) {
    ...
    console.log(`                        $${HARD_CLI_MAX_AMOUNT_USD} hard ceiling, validate-before-sign, and Circle policy`);
    ...
    console.log(`                        cannot exceed $${HARD_CLI_MAX_AMOUNT_USD}. Apify prepaid token is $${APIFY_PREPAID_TOKEN_USD};`);
    console.log("                        an explicit --max-amount below that is refused for Apify picks");
    console.log(`  ${ALLOW_HIGH_MAX_AMOUNT_FLAG}`);
    console.log(`                        Accepted for compatibility; does not raise the $${HARD_CLI_MAX_AMOUNT_USD} ceiling`);
  ...
  // Refuse a YOLO 999 before ranking. $1.05 (Apify token price) is under the
  // $1.10 hard max, so it is judged after the pick: Apify still refuses an
  // explicit cap below the token price; x402 allows any value at or under
  // the hard ceiling. Anything above HARD cannot be payable on any path.
  if (maxAmount != null && maxAmount > HARD_CLI_MAX_AMOUNT_USD) {
    return emitRunError({
      jsonMode,
      error:
        `--max-amount ${maxAmount} exceeds the $${HARD_CLI_MAX_AMOUNT_USD} hard CLI ceiling ` +
        `(nothing was charged)`,
    });
  }
  ...
  if (!parsed.ok) {
    ...
    console.error(fmt.dim(noRunnableHintLine({ payableNow })));
    return 1;
  }
  ...
  // Account for the spend BEFORE handing control to selat-pay: the balance
  // this run read is already consumed, and an interrupt (or a broadcast that
  // then fails) must not leave the pre-payment figure steering the next run.
  // Debiting the enforced --max-amount cap keeps the cache usable while only
  // ever under-stating what is left; see debitCachedGatewayBalance.
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false, intent = "", dryRun = false, jsonMode = false, verbose = false, maxAmount, allowHighMaxAmount = false } = {}) {
  ...
  // that is a hard refuse. The x402 default is not applied here — prepaid
  // uses a known token price, not a per-call x402 cap. An explicit cap
  // above the $1.10 hard CLI ceiling is refused; $1.05 itself fits under it.
  ...
  const underToken = apifyCapBelowTokenError(authorized.capUsd);
  if (underToken) {
    return emitRunError({ jsonMode, error: underToken });
  }
  ...
  // The token buy is a paid selat-pay call — same rule as the x402 path, but
  // its argv is the Apify script's, so the helper falls back to invalidating.
  debitCachedGatewayBalanceForSpawn(await getAgentAddress().catch(() => null), runArgs);
 ...
 * cannot raise the $1.10 ceiling (isTTY is not a human gate).
 */
 ...
 *   - explicit --max-amount is clamped to the hard CLI ceiling (default $1.10).
