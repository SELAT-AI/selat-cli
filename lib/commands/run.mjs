...
export async function run(args) {
  ...
  // opts into buying a replacement token if the current one drains mid-run (see
  // below); every other --flag is dropped (not forwarded to rank), as before.
  let inputInline;
  let inputFile;
  let autoRebuy = false;
  const intentTokens = [];
  ...
  if (!intent) {
    console.error(fmt.error("usage: selat run \"<intent>\" [--input '<json>' | --input-file <path>] [--auto-rebuy]"));
    return 1;
  }
  ...
  if (isApifyPrepaidPick(plan)) {
    return await runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy });
  }
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false }) {
