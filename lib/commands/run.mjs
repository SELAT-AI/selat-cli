...
export async function run(args) {
  ...
  // A payment attempt changes the Gateway balance whatever its outcome — the
  // next chain resolution must read live, not the pre-payment snapshot.
...
async function runApifyPrepaid({ plan, skill, selatPay, inputInline, inputFile, autoRebuy = false, intent = "", dryRun = false, jsonMode = false, verbose = false, maxAmount, allowHighMaxAmount = false } = {}) {
  ...
  // The token buy is a paid selat-pay call — same rule as the x402 path.
  invalidateCircleCache(GATEWAY_BALANCE_KEY_PREFIX);
