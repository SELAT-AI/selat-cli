...
export async function run(args) {
  ...
  // Step 3: execute without a shell. Prepend the bundled .bin to PATH so
  // emitted `selat-pay` commands resolve to the bundled copy when there is
  // no global install.
  const spawnEnv =
    selatPay.source === "bundled"
      ? { PATH: `${selatPay.binDir}${delimiter}${process.env.PATH ?? ""}` }
      : {};
  const run = await sh("selat-pay", selatPayArgs, { inherit: true, env: spawnEnv });
  return run.code;
}
