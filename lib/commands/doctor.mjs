/**
 * `juris doctor` — diagnose setup problems. Single pass through:
...
export async function doctor() {
  console.log(fmt.bold("\njuris doctor — checking your setup"));
  console.log("");
  ...
  if (skill.found) {
    ...
    fail("agent-payment skill not found — `juris run` needs it for discovery and ranking");
  ...
  if (await hasCircle()) {
    ...
    else fail("not authenticated — run `juris init`");
  } else {
    fail("circle CLI not installed — install Circle CLI, then run `juris init`");
  }
  ...
  if (address) {
    ...
    if (bal != null) {
      ...
      else warn("Gateway balance: 0 USDC (run `juris fund` or claim the welcome drip)");
    } else {
      warn("Gateway balance: could not read (Circle API issue or no wallet on the queried chain)");
    }
  } else {
    fail("no agent wallet found — run `juris init`");
  }

  // juris-pay
  console.log(fmt.bold("\njuris-pay:"));
  if (await hasJurisPay()) pass("juris-pay on PATH");
  else fail("juris-pay not on PATH — install or link juris-pay before running `juris run`");

  // Config file
  console.log(fmt.bold("\nConfig:"));
  const cfg = await readConfig();
  if (Object.keys(cfg).length === 0) {
    fail(`${configPath()} missing or empty — run \`juris init\``);
  } else {
  ...
  console.log(fmt.dim("Most fixes: install missing tools, then run `juris init` again."));
  return 1;
}
