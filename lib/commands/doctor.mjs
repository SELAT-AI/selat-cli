...
export async function doctor() {
  ...
  if (!policy.readable) {
    warn("could not read the wallet spending policy (circle wallet limit)");
  } else if (policy.custom) {
    pass(describePolicy(policy));
  } else {
    warn("no custom spending caps on this wallet — a looping agent could drain the balance");
    console.log("  " + fmt.dim("set the hard ceiling (per-tx/daily/weekly/monthly): selat setup-policy"));
  }
