...
import { resolveSelatPay, selatPayVersion, SELAT_PAY_MISSING_MESSAGE } from "../selat-pay.mjs";
...
export async function doctor(args = []) {
  ...
  if (address) {
    ...
    if (bal != null) {
      const fmtBal = usdcAmountText(bal);
    ...
    if ((bal ?? 0) <= 0 && !oc.hasAny) {
      ...
      if (funded) {
        warn(`another agent wallet on this account holds funds: ${funded.address} (Gateway: ${usdcAmountText(funded.usd)})`);
  ...
  if (selatPay.source === "global") {
    ...
    fail(SELAT_PAY_MISSING_MESSAGE);
  }
...
export function gatewayPerChainLines(gw) {
  ...
  return funded.map((c) => {
    ...
    return `${c.network} (domain ${c.domain}): ${usdcAmountText(c.usdc)} — routing detail; pay with --chain ${chainKey}`;
  });
}
