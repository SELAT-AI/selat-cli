...
export async function doctor(args = []) {
  ...
  // Capability catalog — which tier answered, how fresh, and how much of it
  // carries request schemas. This reports exactly what the next `selat search`
  // would rank over, by taking the same path: a fresh per-user cache is read
  // as-is, a cold or expired one is pulled (the pull IS what search would do,
  // and leaves the user warm rather than paying for it on their next command).
  console.log(fmt.bold("\nCapability catalog:"));
  const catalogOut = catalogStatusLines(await fetchCatalogStatus({ skill }));
  const [catalogHead, ...catalogDetail] = catalogOut.lines;
  if (catalogOut.kind === "ok") pass(catalogHead);
  else if (catalogOut.kind === "warn") warn(catalogHead);
  else console.log("  " + fmt.dim(catalogHead));
  ...
  // Agent wallet
  console.log(fmt.bold("\nAgent Wallet:"));
  // Doctor is the diagnostic: every Circle read here is live (a cached value
  // would hide exactly the burst failures this command exists to surface).
  // The balance cache is only INSPECTED, so the user can see what the next
  // `selat run` will steer its --chain by.
  const address = await getAgentAddress({ cache: false });
  if (address) {
    pass(`wallet ${address}`);
    const cacheMeta = circleCacheStatus(gatewayBalanceCacheKey(address));
    const gw = await gatewayBalancesByChain(address, "BASE", { cache: false });
    ...
    for (const line of gatewayCacheLines(cacheMeta, { disabled: circleCacheDisabled() })) console.log("    " + fmt.dim(line));
...
/**
 * One line on the Gateway balance cache `selat run` steers its --chain by
 * (lib/circle-cache.mjs). Doctor itself always reads live — this only tells
 * the user how old the value the next run would reuse is. Pure; exported for
 * tests. `meta` is a circleCacheStatus() result ({ ageMs } or null).
 */
export function gatewayCacheLines(meta, { disabled = false, ttlMs = GATEWAY_BALANCE_TTL_MS } = {}) {
  const ttl = `${Math.round(ttlMs / 60_000)} min`;
  if (disabled) return ["balance cache: off (SELAT_NO_CIRCLE_CACHE) — every `selat run` reads Circle live"];
  if (!meta) return [`balance cache: empty — the next \`selat run\` reads Circle live, then reuses it for ${ttl}`];
  const age = meta.ageMs < 60_000 ? `${Math.round(meta.ageMs / 1000)}s` : `${Math.round(meta.ageMs / 60_000)} min`;
  if (meta.ageMs >= ttlMs) return [`balance cache: ${age} old (expired, ${ttl} ttl) — the next \`selat run\` reads Circle live`];
  return [`balance cache: ${age} old (${ttl} ttl) — \`selat run\` reuses it to pick --chain; this read was live`];
}
...
async function findFundedSibling(currentAddress) {
  try {
    const wallets = await listAgentWallets({ cache: false });
    ...
    const entries = await Promise.all(
      wallets
        ...
        .map(async (w) => ({
          address: w.address,
          usd: (await gatewayBalancesByChain(w.address, "BASE", { cache: false }).catch(() => null))?.total ?? 0
        }))
    );
