...
import { authStatus, getAgentAddress, gatewayBalance, usdcBalances, hasCircle } from "../circle.mjs";
...
export async function doctor() {
  ...
  if (address) {
    ...
    // On-chain USDC across Base/Optimism/Arbitrum (distinct from Gateway).
    ...
    if ((bal ?? 0) <= 0 && !oc.hasAny) {
      warn("No USDC on Base/Optimism/Arbitrum and Gateway is empty — run `selat fund` or claim the welcome drip");
    }
  } else {
  ...
  if (selatPay.source === "global") {
    pass("selat-pay installed (global)");
  } else if (selatPay.source === "bundled") {
    pass(`selat-pay installed (bundled) — ${selatPay.bin}`);
