...
export async function doctor() {
  ...
  if (await hasCircle()) {
    ...
    fail("circle CLI not installed — run `juris init`");
  }
  ...
  else fail("juris-pay not on PATH — run `juris init`");
  ...
  console.log(fmt.dim("Most fixes: run `juris init` again, or follow the suggestions above."));
  return 1;
}
