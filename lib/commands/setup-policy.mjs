...
// Quarantine period — a LOCAL reliability policy, not a Circle wallet write:
// how many days of recent charged 5xx failures (with nothing settled) keep an
// endpoint excluded from ranking picks. Enforced by selat-discovery's
// reliability layer, which reads SELAT_QUARANTINE_DAYS from the shell env or
// the selat config file; the range mirrors its [1,90] clamp (90 = the ledger
// evidence window). No OTP, no wallet, no network.
export const QUARANTINE_DAYS_DEFAULT = "30";
export const QUARANTINE_DAYS_MIN = 1;
export const QUARANTINE_DAYS_MAX = 90;
...
Options:
  ...
  --quarantine-days <n>
                    Local reliability policy (no Circle write, no emailed
                    code): days of recent charged 5xx failures that keep an
                    endpoint excluded from picks (default 30, range 1-90).
                    Saved to the selat config file; given alone, saves it and
                    exits without touching Circle.
...
export function parseSetupPolicyArgs(args = []) {
  const out = { chain: null, help: false, caps: {}, quarantineDays: undefined, unknown: [] };
  const capByFlag = new Map(CAP_FLAGS.map(([key, flag]) => [flag, key]));
  for (let i = 0; i < args.length; i++) {
    const a = args[i];
    if (a === "-h" || a === "--help") { out.help = true; continue; }
    if (a === "--quarantine-days") {
      const v = args[i + 1];
      if (v !== undefined && !String(v).startsWith("--")) { out.quarantineDays = v; i++; }
      else out.quarantineDays = "";
      continue;
    }
    if (a.startsWith("--quarantine-days=")) {
      out.quarantineDays = a.slice("--quarantine-days=".length);
      continue;
    }
    if (a === "--chain") {
      const v = args[i + 1];
...
/**
 * The quarantine period must be a whole number of days in [1, 90] — 0 would
 * disable a protective control and anything past the 90-day evidence window
 * is unenforceable. Returns the error string or null.
 */
export function quarantineDaysError(value) {
  if (value === undefined || value === "" || String(value).startsWith("--")) {
    return "--quarantine-days needs a value (days, 1-90)";
  }
  const n = Number(String(value).trim());
  if (!Number.isInteger(n) || n < QUARANTINE_DAYS_MIN || n > QUARANTINE_DAYS_MAX) {
    return `--quarantine-days must be a whole number between ${QUARANTINE_DAYS_MIN} and ${QUARANTINE_DAYS_MAX}, got "${value}"`;
  }
  return null;
}

/** Persist the quarantine period into the layered selat config (merge-write). */
async function persistQuarantineDays(days) {
  const cfg = await readConfig().catch(() => ({}));
  await writeConfig({ ...cfg, SELAT_QUARANTINE_DAYS: String(days) });
  console.log(fmt.ok(`quarantine period saved: ${days} day(s)`));
  console.log(fmt.dim(`  SELAT_QUARANTINE_DAYS=${days} in ${configPath()}`));
  console.log(fmt.dim("  Endpoints whose recent charged 5xx failures (with nothing settled) fall"));
  console.log(fmt.dim("  inside this window are excluded from ranking picks; older failures still"));
  console.log(fmt.dim("  inform the reliability rate but no longer exclude."));
}
...
export async function setupPolicy(args = []) {
  ...
  // The quarantine period is a LOCAL write (no OTP, no wallet, no network),
  // so a flag value is validated and saved before the Circle-interactive
  // gate: agent harnesses can set it non-interactively, and given alone the
  // command finishes here without touching Circle at all.
  if (parsed.quarantineDays !== undefined) {
    const err = quarantineDaysError(parsed.quarantineDays);
    if (err) {
      console.error(fmt.err(err));
      return 1;
    }
    await persistQuarantineDays(Number(String(parsed.quarantineDays).trim()));
    if (!Object.keys(parsed.caps).length && !parsed.chain) return 0;
  }
  ...
  if (parsed.chain) {
    ...
    // Live read: these chains become the scope of a POLICY write, and a chain
    // missing from a stale listing would be left uncapped.
    const discovered = await listAgentWalletsDetailed({ cache: false }).catch(() => ({ wallets: [] }));
  ...
  // Quarantine period — prompted here (flag already handled and saved above).
  // Default shows the currently configured value so re-running is idempotent.
  if (parsed.quarantineDays === undefined) {
    const cfg = await readConfig().catch(() => ({}));
    const current = cfg.SELAT_QUARANTINE_DAYS || QUARANTINE_DAYS_DEFAULT;
    for (;;) {
      const answer = await prompt("Quarantine period in days (recent-failure window that excludes an endpoint from picks, 1-90)", { default: current });
      const err = quarantineDaysError(answer);
      if (!err) {
        await persistQuarantineDays(Number(String(answer).trim()));
        break;
      }
      console.error(fmt.err(err));
    }
