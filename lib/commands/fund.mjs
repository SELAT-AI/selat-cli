...
import { finiteNumber, formatElapsed, formatUsd as formatUsdc } from "../format.mjs";
import { sessionSpendLine } from "./budget.mjs";
import { FUND_QR_CHAINS, usdcTransferUri, writeFundQr } from "../qr.mjs";

// Money rendering lives in lib/format.mjs so every command agrees; re-exported
// under fund's historical names for callers and tests.
...
export async function fund(args, { interactive = stdinIsInteractive() } = {}) {
  const skill = requireSkill("setup.mjs");
  if (!skill) return 1;
  ...
  const method = parseMethod(flagValue(args, "--method") || "direct");
  if (!method.ok) {
  ...
  const timeout = resolveWaitTimeout(flagValue(args, "--timeout"));
  if (!timeout.ok) {
  ...
  const proceedQuestion = method.value === "eco"
    ? `Proceed with Eco gasless deposit of ${amount} USDC on ${chainArg} (≈${formatUsdc(estimatedEcoNet(amount))} credited after the ~1.5% Eco routing fee)?`
  ...
  if (!wait) {
    ...
    if (method.value === "eco") {
      console.log(fmt.dim(`  Eco routing fee applies — expect ≈${formatUsdc(estimatedEcoNet(amount))} of the ${formatUsdc(amount)} to be credited.`));
    }
...
/**
 * Eco's routing fee, as observed live: a 2.00 USDC eco deposit credited
 * 1.97 USDC in Gateway (~1.5%). Neither the Circle CLI (`circle gateway
 * deposit --help` documents no fee and offers no quote surface) nor the
 * skill's eco path exposes a pre-deposit quote, so this is an ESTIMATE used
 * for disclosure — always rendered with "~"/"≈", never as an exact promise.
 ...
 */
export const ECO_ROUTING_FEE_RATE = 0.015;

/** Estimated NET credited for an eco deposit, floored to a cent so the
 *  disclosure never overpromises. */
export function estimatedEcoNet(amount, rate = ECO_ROUTING_FEE_RATE) {
  const n = finiteNumber(amount);
  if (n == null || n <= 0) return 0;
  return Math.floor(n * (1 - rate) * 100) / 100;
}
...
export function methodPlanLines({ method, chainKey, amount }) {
  if (method === "eco") {
    return [
      ...
      `  fee     ~1.5% Eco routing fee (not quotable pre-deposit — estimate)`,
      `  net     you'll receive ≈${formatUsdc(estimatedEcoNet(amount))} in Gateway`
    ];
  }
  return [
...
export function depositReconcileLine({ method, amount, baselineTotal, currentTotal }) {
  ...
  if (amount - credited > 0.000001) {
    const feeLabel = method === "eco" ? "eco routing fee" : "settlement shortfall";
