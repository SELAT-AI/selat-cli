...
export async function fund(args, { interactive = stdinIsInteractive() } = {}) {
  ...
  const walletAddr = await getAgentAddress().catch((err) => {
    debugError("resolving the agent wallet address", err);
    console.error(fmt.warn(`could not resolve the payer wallet (${errorChain(err)}) — balance reads below are skipped`));
    return null;
  });
    ...
    if (chainCode && onchain != null && onchain < amount) {
      ...
      if (uri) {
        try {
          ...
          debugError("writing the funding QR image", err);
          console.log(fmt.dim(`(couldn't write a QR image: ${errorChain(err)} — the address and payment URI above are enough to pay manually.)`));
        }
      }
      console.log("");
      if (interactive) {
