...
export function readFreeze({ path = freezeFilePath() } = {}) {
  ...
  try {
    ...
    // Fail closed, but don't hide it: an unreadable flag file still freezes
    // payments, and the user needs to know why the metadata is missing.
    debugError("reading the freeze flag file", err);
    console.error(fmt.warn(`freeze flag ${path} is unreadable — payments stay frozen (remove the file to unfreeze: selat unfreeze)`));
