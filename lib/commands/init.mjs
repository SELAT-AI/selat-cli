...
/**
 * Fiat funding offer for an empty wallet: mint a Circle Onramp session via the
 * SELAT catalog and hand the user the browser widget URL. Minting is free and
 * moves no money — the card purchase happens inside Circle's widget, entirely
 * under the user's control — so non-TTY (agent) shells mint and print the URL
 * without prompting, for the agent to relay into chat. Interactive shells get
 * a yes/no first, and a best-effort browser open on yes. Advisory like the
 * rest of the funding check: any failure prints one dim line and never blocks
 * init (the crypto path above still works).
 */
async function offerOnramp(address) {
  const interactive = stdinIsInteractive();
  if (interactive) {
    const yes = await promptYesNo(
      "Buy USDC with a card instead (Circle Onramp — opens in your browser)?",
      { default: true }
    );
    if (!yes) {
      console.log("      " + fmt.dim("Skipped — re-run `selat init` any time for a fresh Onramp link."));
      return;
    }
  }
  let session;
  try {
    ...
    console.log("      " + fmt.dim(`(Circle Onramp unavailable right now: ${e.message} — crypto funding above still works.)`));
    return;
  }
  ...
  if (interactive && (await openInBrowser(session.widgetUrl))) {
