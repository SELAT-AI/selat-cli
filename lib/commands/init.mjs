 ...
 *   6. Check juris-pay
 *   7. Write config to $XDG_CONFIG_HOME/juris-pay/.env
...
export async function init(args) {
  ...
  if (skill.found) {
    ...
    console.error("      " + fmt.dim("Continuing — init can prepare wallet/config, but `juris run` needs the skill."));
  ...
  if (await hasCircle()) {
    ...
    console.error("      Install Circle CLI, authenticate it, then run `juris init` again.");
    return 1;
  }
  ...
  // [6/8] juris-pay
  console.log(fmt.step(6, STEPS, "Checking juris-pay"));
  if (await hasJurisPay()) {
    console.log("      " + fmt.ok("juris-pay already on PATH"));
  } else {
    console.error("      " + fmt.warn("juris-pay not found on PATH."));
    console.error("      " + fmt.dim("Continuing — init will write config; install or link juris-pay before running `juris run`."));
  }
  ...
  console.log(fmt.cyan(`    juris run "summarize the latest news on gold prices"`));
  console.log("");
  console.log("  Set a spending policy before depositing > $20:");
  console.log(fmt.cyan(`    juris setup-policy`));
  console.log("");
  console.log("  If something looks off:");
  console.log(fmt.cyan(`    juris doctor`));
  console.log("");
  return 0;
}
