...
export async function init(args) {
  ...
  // [5/8] Wallets — skip creation when an agent wallet already exists.
  // `circle wallet create` is idempotent, but calling it every run is an
  // unnecessary round-trip and prints noisy output for returning users.
  // `--force` re-runs create regardless.
  console.log(fmt.step(5, STEPS, "Creating agent wallets"));
  let address = force ? null : await getAgentAddress();
  if (address) {
    console.log("      " + fmt.ok("agent wallet already exists; skipping create"));
  } else {
