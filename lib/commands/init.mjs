...
export async function init(args) {
  ...
  // Track the email of the active Circle account so step 5 can reject a "create
  // new" that reuses the same email (which would return the same deterministic
  // wallet). May be undefined if authStatus() couldn't parse it.
  let currentEmail = status.email;
  ...
  // to reuse it or create a new one. Circle agent wallets are deterministic per
  // account (one MPC-derived address per logged-in email), so "create new" means
  // switching to a different Circle account/email — handled by switchAccountAndCreate().
  // `--force` keeps the old non-interactive behavior: re-create on the current
  // account (same deterministic address), no prompt — the escape hatch for scripts/CI.
  ...
  if (force) {
    ...
    if (choice === "use") {
      ...
      address = await switchAccountAndCreate(currentEmail);
      if (!address) return 1;
    }
  } else {
    const created = await createWallets();
...
/**
 * Create a wallet under a different Circle account. Because agent wallet
 * addresses are deterministic per account, the only way to get a different
 * address is to log in with a different email. Returns the new address, or
 * null on login/create failure.
 */
async function switchAccountAndCreate(currentEmail) {
  console.log(
    "      " +
      fmt.warn(
        "A new wallet means a new Circle account. Agent wallet addresses are deterministic per account, so a different address requires a different email."
      )
  );
  console.log(
    "      " +
      fmt.dim(
        "The previous account's wallet (and any funds in it) is not migrated; log back in with its email to reach it again."
      )
  );
  const email = await prompt("Email for the NEW Circle agent account:", {
    validate: (v) => {
      if (!/.@./.test(v)) return "Enter a valid email address.";
      if (currentEmail && v.trim().toLowerCase() === currentEmail.toLowerCase()) {
        return "That is your current account; it would return the same wallet. Use a different email or choose 'use'.";
      }
      return null;
    }
  });
