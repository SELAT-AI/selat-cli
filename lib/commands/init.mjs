...
${fmt.bold("Usage:")}
  selat init [--force] [--email <address>] [--router-url=<url>]
...
export async function init(args) {
  ...
  if (force) {
    ...
    const picked = await chooseAmongWallets({ wallets, configuredAddr });
    if (picked === "new") {
  ...
  if (balance != null && balance > 0) {
    ...
    console.log("      " + fmt.dim("No USDC yet — fund before paid calls: selat fund --amount 2"));
  }
 ...
 */
async function chooseAmongWallets({ wallets, configuredAddr }) {
