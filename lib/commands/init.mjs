...
export async function init(args) {
  ...
  const policy = await spendingPolicy(address).catch((err) => {
    // Still never blocks init, but an unread policy means the "no spending
    // caps" recommendation below is a guess — say which one it is.
    debugError("reading the wallet spending policy", err);
    console.log("      " + fmt.warn(`could not read the wallet spending policy (${errorChain(err)}) — assuming no caps are set`));
...
async function chooseAmongWallets({ wallets, configuredAddr, preset = null }) {
  ...
  await Promise.all(
    wallets.map(async (w) => {
      balances[w.address.toLowerCase()] = await gatewayBalance(w.address).catch((err) => {
        // '?' in the picker rather than a blocked init, but not silent: a
        // wallet whose balance failed to read looks unfunded otherwise.
        debugError(`reading the Gateway balance for ${w.address}`, err);
        console.log("      " + fmt.dim(`(couldn't read the Gateway balance for ${w.address}: ${errorChain(err)})`));
        return null;
      });
    })
  );
