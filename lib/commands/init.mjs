/**
 * `juris-cli init` — bootstrap a new agent payment setup in one flow.
 *
...
export async function init(args) {
  ...
  // [2/7] Circle CLI
  console.log(fmt.step(2, STEPS, "Checking Circle CLI"));
  if (await hasCircle()) {
    console.log("      " + fmt.ok("Circle CLI already on PATH"));
  } else {
    console.error("      " + fmt.err("Circle CLI not found on PATH."));
    console.error("      Install Circle CLI, authenticate it, then run `juris-cli init` again.");
    return 1;
  }
  ...
  // [5/7] juris-pay
  console.log(fmt.step(5, STEPS, "Checking juris-pay"));
  if (await hasJurisPay()) {
    console.log("      " + fmt.ok("juris-pay already on PATH"));
  } else {
    console.error("      " + fmt.warn("juris-pay not found on PATH."));
    console.error("      " + fmt.dim("Continuing — init will write config; install or link juris-pay before running `juris-cli run`."));
  }
    ...
    if (balance != null && balance > 0) {
      ...
      if (wantDrip) {
        ...
        if (dripResult.ok) {
          ...
          console.error("      " + fmt.dim("You can deposit USDC manually: juris-cli fund --chain base --amount 2"));
        }
      } else {
  ...
  console.log(fmt.cyan(`    juris-cli run "summarize the latest news on gold prices"`));
  console.log("");
  console.log("  Set a spending policy before depositing > $20:");
  console.log(fmt.cyan(`    juris-cli setup-policy`));
  console.log("");
  console.log("  If something looks off:");
  console.log(fmt.cyan(`    juris-cli doctor`));
  console.log("");
  return 0;
}
