 ...
 *   8. Funding check (Gateway / on-chain USDC / Circle Onramp fiat offer)
 */

import { fmt, prompt, promptYesNo, stdinIsInteractive } from "../ui.mjs";
import { fetchCatalogStatus, catalogStatusLines } from "../catalog.mjs";
...
// environment. Overrides are rejected unless they are https:// (or http://
// loopback for local development) — MITM is real on plain HTTP because a
// network attacker can rewrite `payTo` in the 402 and steal a signed payment.
...
${fmt.bold("Usage:")}
  selat init [--force] [--email <address>] [--wallet <n|address|new>] [--router-url=<url>] [--install-circle-cli]

${fmt.bold("Options:")}
  --force               Recreate/ensure wallets non-interactively.
  --install-circle-cli  Consent to \`npm install -g @circle-fin/cli@latest\` when the
                        Circle CLI is missing or below ${MIN_CIRCLE_CLI_VERSION}. In a
                        terminal init asks instead; without a TTY it refuses unless
                        this flag is given (a global npm install is a host change).
...
export async function init(args) {
  ...
  const installCircleConsent = args.includes("--install-circle-cli");
  ...
  if ((await hasCircle()) && versionAtLeast(installedCircle, MIN_CIRCLE_CLI_VERSION)) {
    ...
    // Installing or upgrading the Circle CLI is a global npm install — a
    // host change, so it needs consent: ask in a terminal (default yes), or
    // take --install-circle-cli; never do it silently in a non-TTY shell.
    // The install itself moves no money — wallet login (OTP), creation, and
    // funding still happen interactively under the user's own Circle account
    // in the steps below, preserving self-custody.
    const plan = circleInstallPlan({ installed: await hasCircle(), version: installedCircle });
    const what = plan === "upgrade"
      ? `Circle CLI ${installedCircle ?? "(unreadable version)"} is below the ${MIN_CIRCLE_CLI_VERSION} floor`
      : "Circle CLI not found";
    const manual = "npm install -g @circle-fin/cli@latest";
    let consent = installCircleConsent;
    if (!consent && stdinIsInteractive()) {
      consent = await promptYesNo(`      ${what}. Run \`${manual}\` now?`, { default: true });
    }
    if (!consent) {
      console.error("      " + fmt.err(`${what}.`));
      console.error(`      Install it yourself: ${manual}  — then run \`selat init\` again,`);
      console.error("      or re-run with --install-circle-cli to let init do it.");
      return 1;
    }
    console.log("      " + fmt.dim(`${plan === "upgrade" ? "Upgrading" : "Installing"} @circle-fin/cli@latest…`));
  ...
  // Live read: this listing decides which address init WRITES to config, and
  // it runs right after a login that may have switched Circle accounts. A
  // cached list carries no account identity, so reusing one here could
  // persist the previous account's wallet address. Interactive setup, so the
  // extra spawn costs nothing anyone will notice.
  const walletLookup = force ? null : await listAgentWalletsDetailed({ cache: false });
  ...
  // Accept the space form too (`--router-url <url>`, mirroring --email):
  // it used to be silently ignored, writing the DEFAULT router to config for
  // a user who explicitly tried to override it.
  const routerArg = args.find((a) => a.startsWith("--router-url="));
  const spaceIdx = args.indexOf("--router-url");
  const spaceVal = spaceIdx !== -1 && args[spaceIdx + 1] && !args[spaceIdx + 1].startsWith("--")
    ? args[spaceIdx + 1] : "";
  const routerUrl =
    (routerArg ? routerArg.slice("--router-url=".length) : "") || spaceVal ||
    existingConfig.SELAT_ROUTER_URL ||
    DEFAULT_ROUTER_URL;
  // Every paid call goes through this URL, so an unsafe one is fatal rather
  // than a warning: a plaintext or non-http(s) router can capture or rewrite
  // the payment challenge.
  if (!safeHttpUrl(routerUrl)) {
    console.error("      " + fmt.err(`router URL "${routerUrl}" must be an https:// URL (http:// is allowed only for localhost)`));
    console.error("      " + fmt.dim(`pass --router-url=<https url>, or unset SELAT_ROUTER_URL in ${configPath()}`));
    return 1;
  }
  await writeConfig({
    ...existingConfig,
  ...
  if (!routerUrl.startsWith("https://")) {
    console.log("      " + fmt.warn(`router URL is plain HTTP on localhost — development only.`));
  }

  // [8/9] Capability catalog — warm the per-user cache once, here, so the
  // first `selat search` / `selat run` in ANY directory starts warm. The
  // hosted envelope arrives already merged and schema-enriched, which is what
  // lets the ranker quote real request shapes instead of guessing. Best
  // effort: discovery falls back to building the catalog on first use, so a
  // blocked fetch is a note, never a failed init.
  console.log(fmt.step(8, STEPS, "Fetching capability catalog"));
  const catalog = await fetchCatalogStatus();
  const catalogOut = catalogStatusLines(catalog);
  const [catalogHead, ...catalogDetail] = catalogOut.lines;
  console.log("      " + (catalogOut.kind === "ok" ? fmt.ok(catalogHead) : catalogOut.kind === "warn" ? fmt.warn(catalogHead) : fmt.dim(catalogHead)));
  for (const line of catalogDetail) console.log("      " + fmt.dim(line));

  // [9/9] Funding check — report Gateway balance, or nudge to `selat fund`.
  // A null balance means the READ failed, not that the wallet is empty —
  // Circle API failures come in bursts (a run whose step-5 wallet lookup
  // failed once told a funded user "No USDC yet"). Retry once, and if the
  // read still fails, say "unreadable", never "empty".
  console.log(fmt.step(9, STEPS, "Funding check"));
  const balance = await retryNullRead(() => gatewayBalance(address));
  if (balance != null && balance > 0) {
    console.log("      " + fmt.ok(`Gateway balance: ${balance.toFixed(6)} USDC`));
  } else if (balance == null) {
    const [head, ...rest] = gatewayUnreadableLines(address);
    console.log("      " + fmt.warn(head));
    for (const line of rest) console.log("      " + fmt.dim(line));
 ...
 * first, then confirmed-funded (by balance desc), then UNKNOWN (read failed),
 * then confirmed-empty; ties keep original list order. Pure and
 * dependency-free so it's unit-testable. `balances` maps lowercase address →
 * USD number (null/missing = unknown).
 *
 * Unknown ranks ABOVE confirmed-zero: null means the read failed, and Circle
 * API failures come in bursts — a funded wallet whose read failed must not
 * sort below an empty wallet whose read succeeded, because rank #1 is the
 * default a plain Enter (or a non-interactive --wallet-less run) accepts.
...
export function rankWalletCandidates({ wallets, configuredAddr = null, balances = {} }) {
  ...
  // Bands: confirmed-funded (2) > unknown (1) > confirmed-empty (0).
  const band = (usd) => (usd == null ? 1 : usd > 0 ? 2 : 0);
  return (wallets ?? [])
    .map((w, i) => ({
      address: w.address,
      chains: w.chains ?? [],
    ...
    .sort((a, b) =>
      ...
      (band(b.gatewayUsd) - band(a.gatewayUsd)) ||
      ((b.gatewayUsd ?? 0) - (a.gatewayUsd ?? 0)) ||
      (a._i - b._i)
    )
    .map(({ _i, ...w }) => w);
}
...
async function chooseAmongWallets({ wallets, configuredAddr, preset = null }) {
  ...
  await Promise.all(
    wallets.map(async (w) => {
      // gatewayBalance() returns null on failure (it never throws); retry once
      // since Circle API failures come in bursts and this ranking picks the
      // default wallet.
      balances[w.address.toLowerCase()] = await retryNullRead(() => gatewayBalance(w.address));
    })
  );
  ...
  for (let i = 0; i < ranked.length; i++) {
    const w = ranked[i];
    const bal = w.gatewayUsd == null ? "?" : `${w.gatewayUsd.toFixed(6)} USDC`;
  ...
  if (ranked.some((w) => w.gatewayUsd == null)) {
    console.log("      " + fmt.dim(
      "? = the balance read failed (Circle API hiccup) — that wallet may still hold funds; re-run init or check: circle gateway balance --address <0x…> --chain BASE --all"
    ));
  }
...
/**
 * Closing lines when the Gateway balance could not be READ (even after a
 * retry) — distinct from a confirmed-zero balance. Asserting "No USDC yet"
 * here told an already-funded user to fund again; the honest statement is
 * "unreadable, may be funded" plus the re-check command. Pure; exported for
 * tests.
 */
export function gatewayUnreadableLines(address) {
  return [
    "Couldn't read the Gateway balance (Circle API unreachable) — the wallet may already be funded.",
    `Re-check once it recovers: circle gateway balance --address ${address} --chain BASE --all`,
    "If it shows funds you're ready; otherwise fund with: selat fund --amount 2"
  ];
}

/**
 * Empty-wallet funding fork. The wallet is provisioned and config is written —
 * the user now picks how the first USDC arrives, as two symmetric named
 * choices (mirroring the wallet use/new prompt):
 *
 *   send — transfer USDC from an exchange/wallet they already have, to the
 *          agent wallet address (same address on every supported chain);
 *   card — buy USDC in the browser via a Circle Onramp session minted through
 *          the SELAT catalog (free; the card purchase happens inside Circle's
 *          widget, entirely under the user's control).
 *
 * Non-TTY (agent) shells get both paths printed for relay into chat — there's
 * nothing to answer; the user picks by acting on one. Advisory like the rest
 * of the funding check: any failure prints one dim line and never blocks init.
 */
async function offerFunding(address) {
  if (!stdinIsInteractive()) {
    for (const line of noUsdcHintLines()) console.log("      " + fmt.dim(line));
    for (const line of sendUsdcLines(address)) console.log("      " + fmt.dim(line));
    await printOnrampOffer(address, { open: false });
    return;
  }
  console.log("      " + fmt.warn("No USDC yet — the wallet needs funds before paid calls."));
  console.log("      How do you want to add it?");
  console.log(`        ${fmt.bold("send")} — transfer USDC from an exchange or wallet you already have`);
  console.log(`        ${fmt.bold("card")} — buy USDC in your browser (Circle Onramp)`);
  console.log(`        ${fmt.bold("skip")} — decide later (selat fund walks funding any time)`);
  const answer = await prompt("Fund via [send/card/skip]", {
    default: "send",
    validate: (v) =>
      ["send", "card", "skip"].includes(v.trim().toLowerCase())
        ? null
        : "Type 'send', 'card', or 'skip'."
  });
  const choice = answer.trim().toLowerCase();
  if (choice === "skip") {
    console.log("      " + fmt.dim("Skipped — fund any time: selat fund --amount 2 (crypto) or selat fund --onramp (card)"));
    return;
  }
  if (choice === "card") {
    await printOnrampOffer(address, { open: true });
    return;
  }
  const lines = sendUsdcLines(address);
  console.log("      " + fmt.cyan(lines[0]));
  console.log("      " + fmt.cyan(lines[1]));
  for (const line of lines.slice(2)) console.log("      " + fmt.dim(line));
}

/**
 * Mint a scoped Circle Onramp session and print the widget URL (plus a
 * best-effort browser open for the interactive card choice). Minting is free
 * and moves no money. Never throws — a mint failure is one dim line and the
 * send path still works.
 */
async function printOnrampOffer(address, { open }) {
  let session;
  try {
    session = await createOnrampSession({ address, userId: "selat-cli-init" });
  } catch (e) {
    console.log("      " + fmt.dim(`(Circle Onramp unavailable right now: ${e.message} — send USDC instead: selat fund)`));
    return;
  }
...
/**
 * The `send` funding path: the agent wallet address plus the chains a
 * transfer can safely arrive on. The address is the same SCA on every listed
 * chain, but the list is a real boundary — USDC sent on a chain outside it
 * has no deposit path into Gateway from this CLI and can strand at the
 * address. Pure; exported for tests.
 */
export function sendUsdcLines(address, chains = FUND_QR_CHAINS) {
  return [
    "Send USDC to your agent wallet address (same address on every listed chain):",
    `  ${address}`,
    `Chains: ${chains.map((c) => c.key).join(", ")} — only these; elsewhere funds can strand.`,
    "Once it lands, deposit into Gateway: selat fund --amount 2",
    "(selat fund also shows a QR / payment URI for the transfer.)"
  ];
}
