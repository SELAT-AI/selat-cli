...
export async function search(args) {
  ...
  // --pick: the ranker's own footer advertises it ("Use --pick for a runnable
  // payment plan on the top match"), but `selat search` silently dropped the
  // flag and printed the plain shortlist again. Forward it: the ranker prints
  // the top pick's plan as JSON (still free — a plan is never a settlement).
  if (args.includes("--pick")) {
    const res = await sh("node", [rank, intent, "--pick", ...passthrough], { inheritStdout: true });
    replayStderr(res);
    return res.code;
  }
 ...
 */
/**
 * Footer for a top pick the ranker refused to hand a command for because its
 * indexed URL is missing required params (`plan.missingQueryParams`). Pure and
 * exported for tests. `[]` when the plan names no missing params — an unrelated
 * missing exec hint stays silent, as before.
 */
export function paramsNextStepLines(plan, runCmd) {
  const missing = Array.isArray(plan?.missingQueryParams) ? plan.missingQueryParams.filter(Boolean) : [];
  if (!missing.length) return [];
  const byName = new Map((plan?.queryParams ?? []).map((p) => [p?.name, p]));
  const service = plan?.service?.name ? `${plan.service.name} ` : "";
  const flags = missing.map((name) => `--param ${name}=<${name}>`).join(" ");
  const lines = [
    "",
    fmt.bold(`▸ Next: ${service}needs ${missing.length === 1 ? "a parameter" : "parameters"} before it can be probed or paid`),
  ];
  for (const name of missing) {
    const p = byName.get(name);
    const desc = p?.description ? ` — ${p.description}` : "";
    lines.push(fmt.dim(`  ${name}${p?.type ? ` (${p.type})` : ""}${desc}`));
  }
  lines.push(fmt.cyan(`  ${runCmd} ${flags} --dry-run`));
  lines.push(fmt.dim("  --dry-run shows the pick, price, and command without paying; drop it to pay."));
  const docsUrl = plan?.docs?.docsUrl;
  if (docsUrl) lines.push(fmt.dim(`  params documented at: ${docsUrl}`));
  return lines;
}
...
async function printProbeNextStep(rank, intent, capabilityArgv = []) {
  try {
    ...
    if (!argv) {
      // No exec hint at all. The common reason is a top pick whose indexed URL
      // needs params it doesn't carry (`?ticker=…`): the ranker withholds the
      // command so nobody pays for an error response. Saying nothing here left
      // the funnel dead-ended — name the params and the run flag instead.
      for (const line of paramsNextStepLines(plan, runCmd)) console.log(line);
      return;
    }
    if (argv[0] !== "selat-pay") return;
