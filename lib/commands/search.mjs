 ...
 * --json. No payment path exists in this command — rank.mjs without --pick never settles.
 */
...
${fmt.bold("Usage:")}
  selat search "<intent or capability>" [flags]
...
${fmt.cyan("selat-pay … --probe-only")} command printed below the results.`;
...
export async function search(args) {
  ...
  // value-taking flags (e.g. the "3" in `--top 3`) so they don't pollute the query.
  const valueFlags = new Set(["--top"]);
  const words = [];
  ...
  if (!intent) {
    console.error(fmt.error("usage: selat search \"<intent or capability>\""));
    return 1;
  }
  ...
  if (topIdx !== -1 && args[topIdx + 1]) passthrough.push("--top", args[topIdx + 1]);
  if (args.includes("--refresh")) passthrough.push("--refresh");
  if (args.includes("--explain")) passthrough.push("--explain-payability");

  // --json: forward verbatim for machine consumers (agents, interception hooks).
  if (args.includes("--json")) {
    const res = await sh("node", [rank, intent, "--json", ...passthrough], { inherit: true });
    return res.code;
  }

  // Human mode: stream the ranked list, then append a probe-first next step.
  const list = await sh("node", [rank, intent, ...passthrough], { inherit: true });
  if (list.code !== 0) {
    // rank.mjs failed (it streamed its own error). If discovery couldn't reach the
    // catalog because the host sandboxes egress, surface the exact allowlist fix.
    if (await egressLikelyBlocked()) console.error("\n" + fmt.dim(sandboxHintText()));
    return list.code;
  }

  await printProbeNextStep(rank, intent);
  return 0;
}

/**
 * Derive a free `selat-pay … --probe-only` line for the top match, so search → probe →
 ...
 */
async function printProbeNextStep(rank, intent) {
  try {
    const pick = await sh("node", [rank, intent, "--pick"]);
    ...
    if (hint?.flow === "apify-prepaid-token") {
      ...
      console.log(fmt.cyan("  selat run \"" + intent + "\""));
      console.log(fmt.dim("  Buys one ~$1 token, then runs the Actor via Bearer; the token is reused for"));
      console.log(fmt.dim("  later Apify calls until it drains or expires. Don't selat-pay POST the Actor URL."));
      return;
    }
    ...
    console.log("");
    console.log(fmt.bold("▸ Next (free probe, no spend):"));
    console.log(fmt.cyan("  " + argv.join(" ")));
    console.log(fmt.dim("  Then drop --probe-only to pay, or run: selat run \"" + intent + "\""));
  } catch {
