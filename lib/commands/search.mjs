...
import { positionalWords } from "../args.mjs";
...
export async function search(args) {
  ...
  const intent = positionalWords(args, { valueFlags: ["--top"] }).join(" ").trim();
  if (!intent) {
    console.error(fmt.error("usage: selat search \"<intent or capability>\""));
    return 1;
  }

  const skill = requireSkill("rank.mjs", { message: "discovery skill not found" });
  if (!skill) return 1;
  const rank = skillScript(skill, "rank.mjs");
