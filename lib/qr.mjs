 ...
 *
 * Membership rule: a chain is fundable iff it is BOTH
 *   (a) Gateway-supported (deposit path into the unified balance) —
 *       https://developers.circle.com/gateway/references/supported-blockchains
 *   (b) Agent-Wallet-supported (the SCA is registered there, so the address is
 *       user-controlled; also what gives the Circle CLI a --chain code) —
 *       https://developers.circle.com/agent-stack/agent-wallets/supported-blockchains
 *       (canonical live list: `circle blockchain list`)
 *
 * Known exclusions (verified 2026-08-12) and which side excludes them:
 *   - HyperEVM, Sei, Sonic, World Chain: Gateway yes, Agent Wallet NO — USDC
 *     sent to the wallet address there lands at an address the user cannot
 *     control. Never list without (b).
 *   - Monad: Agent Wallet yes (hence policy rows), Gateway NO.
 *   - Solana: Gateway yes, but non-EVM — outside the EVM funding path.
 * Adding a chain that newly satisfies both: one entry here + re-verify the
 * Onramp widget actually carries it (see lib/onramp.mjs — the widget silently
 * drops optimism), then update test/fund-qr.test.mjs's EXPECTED_CHAINS.
 */
...
export function fundQrPath(chainKey, { dir = join(stateHome(), "selat-pay") } = {}) {
