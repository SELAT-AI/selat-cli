...
export function probeArgvForPlan(plan) {
  ...
  // A hint could inject --probe-only (which would make the "paid" leg exit 0
  // without settling — a fake success). Strip it here; the probe path adds its
  // own, the settle path must never carry one.
  payArgv = stripFlag(payArgv, "--probe-only");
