...
export function ensureHarnessPath() {
  ...
  try {
    ...
    debugError("resolving this runner's own path", err);
    return;
  }
  if (!self) return;
  ...
  for (const h of HARNESS_BIN_DIRS) {
    try {
      ...
      /* never block a command over PATH linking — but leave a debug trail */
      debugError(`linking selat onto PATH for ${h.name}`, err);
    }
  }
}
...
export async function egressLikelyBlocked() {
  ...
  try {
    ...
    debugError("probing catalog egress", err);
