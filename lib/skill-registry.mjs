...
// Skill *content* lives in a separate repo (SELAT-AI/selat-skills); this CLI
// only lists, installs, and runs skills. Skills are installed on demand into
// the user skills dir. The repo is public, so install-on-demand fetches the
// manifest anonymously over raw.githubusercontent.com (no auth needed); if that
// fails (e.g. the repo is made private, or a network/proxy blocks raw), it
// falls back to `gh api` using the user's GitHub CLI auth. For local
// development, point SELAT_SKILLS_DIR at a checkout of the repo to skip the network.
...
/**
 * Fetch a file from the selat-skills repo. Tries the public raw URL first
 * (anonymous, no auth) and falls back to `gh api` (authenticated) so the same
 * code keeps working if the repo is later made private. Returns raw contents.
 */
async function fetchSkillFile(repoPath) {
  // 1) Anonymous public fetch over raw.githubusercontent.com.
  const rawUrl = `${SKILLS_RAW_BASE}/${SKILLS_REPO}/${SKILLS_REF}/${repoPath}`;
  try {
    const res = await fetch(rawUrl);
    if (res.ok) return await res.text();
  } catch {
    // network error — fall through to gh
  }

  // 2) Authenticated fallback via the GitHub CLI (works for private repos).
  const r = await sh("gh", [
    "api",
    `repos/${SKILLS_REPO}/contents/${repoPath}?ref=${SKILLS_REF}`,
    "-H",
    "Accept: application/vnd.github.raw"
  ]).catch((e) => ({ code: 1, stderr: String(e?.message ?? e), stdout: "" }));
  if (r.code !== 0) {
    const hint = (r.stderr || "").trim() ||
      "tried raw.githubusercontent.com and gh — check the skill name, or `gh auth login` for a private repo";
    throw new Error(`could not fetch ${repoPath} from ${SKILLS_REPO}: ${hint}`);
  }
  return r.stdout;
}
