...
import { debitCachedGatewayBalanceForSpawn, getAgentAddress, resolveFundedChainKey } from "./circle.mjs";
import { assertSafeHttpUrl } from "./url-safety.mjs";
import { frozenMoneyMove } from "./commands/freeze.mjs";
import { missingSessionBudget } from "./commands/budget.mjs";
import { authorizeExplicitMaxAmount } from "./spend-guard.mjs";
...
function assertSafePaymentUrl(url, { source = "step" } = {}) {
  return assertSafeHttpUrl(url, { source });
}
...
export function compileStep(manifest, step, params, overrides = {}, defaultChain = null) {
  ...
  // An explicit --max-amount (override) is still bounded by the hard CLI
  // ceiling (default $1.10). isTTY / --allow-high-max-amount cannot raise it.
  // A cap that comes from the untrusted manifest/step stays bounded by the
  // manifest ceiling.
  let maxAmount;
...
export async function invokeSkill(name, cliParams = {}, { overrides = {}, onStep, continueOnError = true, capture = false } = {}) {
  ...
  if (frozen) {
    const err = new Error(frozen.error);
    err.frozen = true;
    throw err;
  }
  const unarmed = missingSessionBudget();
  if (unarmed) {
    const err = new Error(unarmed.error);
    err.missingSessionBudget = true;
    err.hint = unarmed.hint;
    throw err;
  }
  ...
  for (const [index, step] of skill.manifest.steps.entries()) {
    ...
    try {
      ...
      // Every step is a payment attempt: account for its cap before spawning,
      // so an interrupted step cannot leave a pre-payment balance behind.
      debitCachedGatewayBalanceForSpawn(await getAgentAddress().catch(() => null), spawnArgs);
 ...
 */
export async function verifySkillDir(dir, { params = {}, overrides = {}, pay = false, liveProbe = false, onStep } = {}) {
  if (!liveProbe) {
    throw new Error("skill verification may invoke each step endpoint; pass liveProbe: true only after confirming the targets are safe to call");
  }
  ...
  for (const [index, step] of manifest.steps.entries()) {
    ...
    // Phase 1: explicitly acknowledged live probe (never settles).
    if (onStep) onStep({ index, total, phase: "probe", step, display });
    const probe = selatPaySpawn(selatPay, [...argv, "--probe-only", "--live-probe"]);
    ...
    if (pay && reachable && withinCap) {
      const frozen = frozenMoneyMove("pay");
      if (frozen) throw new Error(frozen.error);
      const unarmed = missingSessionBudget();
      if (unarmed) throw new Error(unarmed.error);
  ...
  // `receipt` is the canonical selat-verify/v1 shape (also the on-disk file and
  // what `skill submit` reads); expose it so `verify --json` emits exactly that.
  return { ok, steps, receiptPath, name: manifest.name, receipt };
}
