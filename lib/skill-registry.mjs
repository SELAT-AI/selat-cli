...
import { GATEWAY_BALANCE_KEY_PREFIX, getAgentAddress, resolveFundedChainKey } from "./circle.mjs";
...
export async function invokeSkill(name, cliParams = {}, { overrides = {}, onStep, continueOnError = true, capture = false } = {}) {
  ...
  for (const [index, step] of skill.manifest.steps.entries()) {
    ...
    try {
      ...
      // Every step is a payment attempt: drop the cached Gateway balance
      // whatever the outcome so the next auto-resolved chain reads live.
