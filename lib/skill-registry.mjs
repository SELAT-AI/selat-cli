...
// the user skills dir. The repo is private, so install-on-demand fetches via
// `gh api` (the user's authenticated GitHub CLI). For local development, point
// SELAT_SKILLS_DIR at a checkout of the selat-skills repo to skip the network.
...
/**
 * Fetch a file from the (private) selat-skills repo via the GitHub CLI, which
 * uses the user's existing `gh` auth. Returns the raw file contents.
 */
async function ghFetchFile(repoPath) {
  const r = await sh("gh", [
    "api",
  ...
  if (r.code !== 0) {
    const hint = (r.stderr || "").trim() || "is the GitHub CLI installed and authenticated? (gh auth login)";
...
export async function listAvailable() {
  ...
  if (dev && existsSync(join(dev, "index.json"))) {
    ...
    raw = await ghFetchFile("index.json");
  }
  let parsed;
  try {
    parsed = JSON.parse(raw);
...
export async function installSkill(src, { force = false } = {}) {
  ...
  if (looksLikePath(src)) {
    ...
    if (devPath && existsSync(devPath)) {
      ...
      raw = await ghFetchFile(`skills/${src}/manifest.json`);
      source = SKILLS_REPO;
    }
  }
