...
export function validateManifest(manifest, { dir } = {}) {
  ...
  for (const [i, step] of manifest.steps.entries()) {
    ...
    if (step.maxAmount != null) {
      parseMaxAmount(step.maxAmount, { source: `step ${i} maxAmount`, bounded: true });
    }
  }
  if (manifest.maxAmount != null) {
    parseMaxAmount(manifest.maxAmount, { source: "manifest maxAmount", bounded: true });
  }
 ...
 * auto-verify CI. Best-effort: returns { generatedAt, byName: Map<name, entry> }
 * where entry = { status, reachable, total, livePriceUsd, checkedAt }. Returns an
 * empty map if the registry is absent or unparseable, so callers degrade cleanly.
 */
export async function fetchReliability() {
  const empty = { generatedAt: null, byName: new Map() };
  let raw;
  try {
    const dev = devSkillsRoot();
  ...
  return { generatedAt: parsed.generatedAt ?? null, byName };
}
...
export async function verifySkillDir(dir, { params = {}, overrides = {}, pay = false, onStep } = {}) {
  ...
  const env = selatPay.source === "bundled"
    ? { PATH: `${selatPay.binDir}${delimiter}${process.env.PATH ?? ""}` }
    : {};
  ...
  for (const [index, step] of manifest.steps.entries()) {
    const { argv, display } = compileStep(manifest, step, resolved, overrides);
    const url = argv[1];
    ...
    const pr = await sh("selat-pay", [...argv, "--probe-only"], { env });
    ...
    if (pay && reachable && withinCap) {
      ...
      const t0 = Date.now();
      const rr = await sh("selat-pay", argv, { env });
 ...
 * Otherwise: branch -> register (writes index.json) -> commit -> push origin ->
 * gh pr create. Falls back with hints if push/PR fails (e.g. no write access).
 */
export async function submitSkill(dir, { dryRun = false, repo = SKILLS_REPO } = {}) {
  const manifest = await loadManifest(dir);
  ...
  if ((await sh("git", ["rev-parse", "--is-inside-work-tree"], { cwd: repoRoot }).catch(() => ({ code: 1 }))).code !== 0) {
    throw new Error(`${repoRoot} is not a git checkout of ${repo}; clone it and place the skill under skills/${name}/`);
  }
  const cb = await sh("git", ["checkout", "-b", branch], { cwd: repoRoot });
  if (cb.code !== 0) await sh("git", ["checkout", branch], { cwd: repoRoot });
  await registerSkill(dir); // writes index.json on the branch
  await sh("git", ["add", relSkill, "index.json"], { cwd: repoRoot });
  const c = await sh("git", ["commit", "-m", title], { cwd: repoRoot });
  if (c.code !== 0 && !/nothing to commit/i.test((c.stdout || "") + (c.stderr || ""))) {
    throw new Error(`git commit failed: ${((c.stderr || c.stdout) || "").trim()}`);
  }
  const p = await sh("git", ["push", "-u", "origin", branch], { cwd: repoRoot });
  if (p.code !== 0) {
    return { pushed: false, branch, repoRoot,
      hint: `push to origin failed (no write access?). Fork and PR from your fork:\n  gh repo fork ${repo} --remote\n  git push -u <your-fork-remote> ${branch}\n  gh pr create --repo ${repo} --head <you>:${branch}`,
      detail: (p.stderr || "").trim() };
  }
  const pr = await sh("gh", ["pr", "create", "--repo", repo, "--head", branch, "--title", title, "--body", body], { cwd: repoRoot });
  if (pr.code !== 0) {
    return { pushed: true, branch, prUrl: null, hint: `branch pushed; open the PR manually: gh pr create --repo ${repo} --head ${branch}`, detail: (pr.stderr || "").trim() };
  }
  return { pushed: true, branch, prUrl: (pr.stdout || "").trim() };
}
