...
// (overrides.maxAmount) is the authority and may exceed this ceiling. At
// install time an interactive user may also accept above-ceiling declared caps
// after seeing them (planCapConfirmation + installSkill's onConfirmCaps).
export const MANIFEST_MAX_AMOUNT_CEILING_USD = 1.0;

/**
 * Pure install-path decision for manifest-declared spend caps above
 * MANIFEST_MAX_AMOUNT_CEILING_USD. Returns { action, caps } where caps lists
 * every above-ceiling declared cap ({ source, label, cap }) and action is:
 *   - "proceed": nothing above the ceiling (or an explicit --max-amount was
 *     given — the user is the authority and validateManifest enforces it
 *     exactly as before), so install without asking;
 *   - "confirm": above-ceiling caps and stdin can answer a prompt — show the
 *     offending steps and ask (default NO);
 *   - "reject": above-ceiling caps but no way to ask (non-TTY) — keep today's
 *     fail-fast validateManifest error, which names the --max-amount override.
 * Operates on the raw parsed manifest defensively: only finite numeric caps
 * count as above-ceiling here; malformed values still fail validateManifest.
 */
export function planCapConfirmation(manifest, { maxAmountOverride = null, interactive = false, ceiling = MANIFEST_MAX_AMOUNT_CEILING_USD } = {}) {
  if (maxAmountOverride != null) return { action: "proceed", caps: [] };
  const caps = [];
  const steps = Array.isArray(manifest?.steps) ? manifest.steps : [];
  for (const [i, step] of steps.entries()) {
    if (step?.maxAmount == null) continue;
    const n = Number(step.maxAmount);
    if (Number.isFinite(n) && n > ceiling) {
      caps.push({ source: `step ${i + 1}`, label: step?.label ?? null, cap: n });
    }
  }
  if (manifest?.maxAmount != null) {
    const n = Number(manifest.maxAmount);
    if (Number.isFinite(n) && n > ceiling) {
      caps.push({ source: "manifest", label: null, cap: n });
    }
  }
  if (caps.length === 0) return { action: "proceed", caps };
  return { action: interactive ? "confirm" : "reject", caps };
}
...
/**
 * Wrap a spawn helper so a spawn failure comes back as a result (code null,
 * reason in stderr) instead of rejecting — the git/gh probes below only need
 * "did it succeed", but throwing the reason away made a missing `gh` look like
 * a permission answer. Mirrors shSafe() for the injectable `run` in
 * submitSkill().
 */
function safely(run) {
  return async (...args) => {
    try {
      ...
      debugError(`spawning ${args[0]}`, err);
      return { code: null, signal: null, stdout: "", stderr: errorMessage(err) };
    }
  };
}

/** Returns { skills, broken } — broken is [{ name, dir, source, error }]. */
async function readSkillsFrom(baseDir, source) {
  if (!existsSync(baseDir)) return { skills: [], broken: [] };
  let entries;
  try {
    entries = await readdir(baseDir, { withFileTypes: true });
  } catch (err) {
    debugError(`listing skills in ${baseDir}`, err);
    return { skills: [], broken: [{ name: null, dir: baseDir, source, error: err.message ?? String(err) }] };
  }
  const out = [];
  const broken = [];
  for (const ent of entries) {
    ...
    try {
      ...
      // A malformed folder must not fail the whole list, but it must not vanish
      // either: an installed skill silently missing from `selat skill list` is
      // indistinguishable from one that was never installed.
      debugError(`loading skill manifest in ${dir}`, err);
      broken.push({ name: ent.name, dir, source, error: err.message ?? String(err) });
    }
  }
...
export async function listSkills() {
  return (await listSkillsDetailed()).skills;
}

/**
 * listSkills() plus the folders that could not be loaded, so the command layer
 * can tell the user a skill is installed-but-broken instead of dropping it.
 * Returns { skills, broken }.
 */
export async function listSkillsDetailed() {
  ...
  if (dev) {
    const found = await readSkillsFrom(join(dev, "skills"), "dev");
    for (const s of found.skills) byName.set(s.name, s);
    broken.push(...found.broken);
  }
  const user = await readSkillsFrom(userSkillsDir(), "user");
  for (const s of user.skills) byName.set(s.name, s); // installed wins
  broken.push(...user.broken);
  return {
    skills: [...byName.values()].sort((a, b) => a.name.localeCompare(b.name)),
    broken: broken.filter((b) => !byName.has(b.name))
  };
}

/**
 * Resolve a single installed skill by name (user dir first, then dev dir).
 * Throws when the skill IS installed but its manifest cannot be loaded — the
 * caller's "not installed, run `selat skill install`" message would otherwise
...
export async function resolveSkill(name) {
  ...
  if (bad) throw new Error(`skill '${name}' is installed at ${bad.dir} but could not be loaded: ${bad.error}`);
  return null;
}
...
export async function fetchReliability() {
  ...
  try {
    ...
    debugError("fetching the skill reliability registry", err);
  ...
  try {
    ...
    debugError("parsing the skill reliability registry", err);
    return empty;
  }
  const byName = new Map();
  const byUrl = new Map();
 ...
 *
 * `onConfirmCaps` (optional, async (caps) => boolean) is the interactive
 * install path for a manifest whose declared caps exceed
 * MANIFEST_MAX_AMOUNT_CEILING_USD: it is shown the offending caps and its
 * answer decides whether to accept them (validate unbounded) or abort. When it
 * is absent (non-TTY), validation keeps today's fail-fast ceiling error. An
 * explicit `maxAmount` (--max-amount) bypasses it entirely — the user is
 * already the authority and validateManifest enforces the override as before.
 */
export async function installSkill(src, { force = false, maxAmount, onConfirmCaps } = {}) {
  ...
  // Above-ceiling declared caps: ask an interactive user (via onConfirmCaps)
  // before accepting them; without a way to ask, fall through so
  // validateManifest raises today's fail-fast ceiling error.
  let acceptDeclaredCaps = false;
  const plan = planCapConfirmation(parsed, {
    maxAmountOverride: maxAmount,
    interactive: typeof onConfirmCaps === "function"
  });
  if (plan.action === "confirm") {
    acceptDeclaredCaps = await onConfirmCaps(plan.caps);
    if (!acceptDeclaredCaps) {
      throw new Error(
        `install cancelled — declared cap(s) above $${MANIFEST_MAX_AMOUNT_CEILING_USD} were not accepted; ` +
          `re-run with an explicit --max-amount <usd> to authorize a specific cap`
      );
    }
  }
  ...
  const manifest = validateManifest(parsed, {
    maxAmountOverride: maxAmount,
    boundMaxAmounts: !acceptDeclaredCaps
  });
 ...
 */
export async function invokeSkill(name, cliParams = {}, { overrides = {}, onStep, continueOnError = true, capture = false } = {}) {
  ...
  for (const [index, step] of skill.manifest.steps.entries()) {
    ...
    try {
      ...
      // capture: hold each step's streams instead of inheriting the terminal —
      // stdout is the paid response payload, stderr is selat-pay diagnostics.
      const res = await sh(cmd, spawnArgs, { inherit: !capture });
      results.push({
        ...base,
        code: res.code,
        ok: res.code === 0,
        ...(capture ? { stdout: res.stdout ?? "", stderr: res.stderr ?? "" } : {})
      });
    } catch (err) {
...
export function parseProbeStdout(stdout) {
  ...
  try { json = JSON.parse(stdout.trim()); }
    ...
    // selat-pay may interleave log lines with the probe payload; retry on the
    // outermost braces before giving up.
    const a = stdout.indexOf("{"), b = stdout.lastIndexOf("}");
    if (a !== -1 && b > a) {
      try { json = JSON.parse(stdout.slice(a, b + 1)); }
      catch (err) { debugError("parsing probe stdout", err); }
    }
  }
...
export async function submitSkill(dir, { dryRun = false, repo = SKILLS_REPO, fork = null, run = sh } = {}) {
  ...
  if ((await runSafe("git", ["rev-parse", "--is-inside-work-tree"], { cwd: repoRoot })).code !== 0) {
  ...
  const head = await runSafe("git", ["rev-parse", "--abbrev-ref", "HEAD"], { cwd: repoRoot });
  ...
  if (fork === false) via = "origin";
    ...
    const perm = await runSafe("gh", ["api", `repos/${repo}`, "--jq", ".permissions.push"], { cwd: repoRoot });
...
async function pushViaFork({ repo, branch, repoRoot, run = sh }) {
  ...
  const who = await runSafe("gh", ["api", "user", "--jq", ".login"], { cwd: repoRoot });
  ...
  const originUrl = ((await runSafe("git", ["remote", "get-url", "origin"], { cwd: repoRoot })).stdout || "").trim();
  ...
  let pushTarget = "fork";
  const existing = await runSafe("git", ["remote", "get-url", "fork"], { cwd: repoRoot });
  if (existing.code !== 0) {
