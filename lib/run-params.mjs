 ...
 *   3. for POST/PUT/PATCH hints that carry --body, remaining keys merge into
...
export function applyParamsToPayArgs(argsIn, params, plan = {}) {
  ...
  const pathPass = applyParamsToUrl(args[1], params.filter(([k]) => {
    ...
    return base.includes(`{${k}}`) || base.includes(`%7B${k}%7D`) || new RegExp(`/:${k}\\*?(?=/|$)`).test(base);
  }));
  args[1] = pathPass.url;
