...
/** Absolute path of a script inside the resolved skill's `scripts/` dir. */
export function skillScript(skill, ...segments) {
  return join(skill.path, "scripts", ...segments);
}

/**
 * findSkill() plus the shared "not installed" report every command printed by
 * hand: the error line, a blank line, then the install instructions in cyan.
 * Returns the skill when found, or null after reporting — so callers read
 * `const skill = requireSkill(...); if (!skill) return 1;`.
 */
export function requireSkill(requiredScript = "rank.mjs", { message = "agent-payment skill not found" } = {}) {
  const skill = findSkill(requiredScript);
  if (skill.found) return skill;
