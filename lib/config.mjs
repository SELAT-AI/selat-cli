...
export async function readConfig() {
  try {
    ...
    // Anything else (a directory in the file's place, no read permission, a bad
    // symlink) is a real failure. Keep the cause but name the path, so callers
    // that only expected "not configured yet" can't mistake it for that.
    throw new Error(`could not read ${configPath()}: ${err.message}`, { cause: err });
  }
}
