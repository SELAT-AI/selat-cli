 ...
 *
 * Per-call ceiling: $1.10 for everyone (aligned with selat-pay
 * HARD_PAY_MAX_AMOUNT_USD). `stdin.isTTY` is not a human gate — Cursor,
 * Claude Code, Codex, Gemini CLI, OpenClaw, and Hermes often allocate a
 * PTY, so a TTY confirm or `--allow-high-max-amount` is agent-spoofable.
 ...
 * other ceilings.
 *
 * The Apify prepaid token ($1.05) fits under this hard max. The constant
 * stays as the documented token price for runApifyPrepaid messaging and
 * floor checks (refuse an explicit --max-amount below $1.05 for Apify
 * picks) — not a raise path above the hard max, and not a generic hole.
 *
...
export const HARD_CLI_MAX_AMOUNT_USD = 1.1;
...
// It is not a raise path. The hard CLI ceiling is $1.10.
...
/**
 * Allowed per-call ceiling. Always $1.10. interactive / allowHigh / harness /
 * apifyPrepaid are accepted and ignored — they cannot raise. A PTY agent
 * passing --allow-high-max-amount still gets $1.10. Apify's $1.05 token
 * price is a floor on explicit caps for that pick, not a higher ceiling.
 */
...
/**
 * Apify prepaid-token floor. An explicit --max-amount below the known
 * $1.05 token price is refused (nothing was charged). Returns null when
 * the cap is absent or meets the token price.
 */
export function apifyCapBelowTokenError(capUsd) {
  if (capUsd == null) return null;
  if (capUsd < APIFY_PREPAID_TOKEN_USD) {
    return `Apify prepaid-token purchase is $${APIFY_PREPAID_TOKEN_USD} — --max-amount ${capUsd} is below that; nothing was charged`;
  }
  return null;
}
 ...
 * `apifyPrepaid` applies the $1.05 token-price floor (not a raise).
 */
export function authorizeExplicitMaxAmount(explicit, {
  ...
  if (apifyPrepaid) {
    const under = apifyCapBelowTokenError(n);
    if (under) return { ok: false, error: under };
  }
 ...
 * exceed $1.10. Apify's $1.05 token price is a floor, not a raise.
 */
